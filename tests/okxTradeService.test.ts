...
describe('OkxTradeService.mapToOKXPayload', () => {
  ...
  it('附带止盈止损：映射结构', () => {
    ...
    // 主单 ordType=limit 命中 SL_AS_MARKET_ORDER_TYPES 白名单 → 止损委托价按市价 '-1'
    // （有意设计，见 okxTradeService.ts SL_AS_MARKET_ORDER_TYPES；2026-09-12 测试对齐）
    assert.equal(algo.slOrdPx, '-1');
    assert.equal(algo.slTriggerPxType, 'last');

    // 显式传入 slOrdPx（"止损幅度:触发价;委托价"拆开）时优先透传，不走市价白名单
    const payload2 = (svc as any).mapToOKXPayload({
      ...params,
      attachAlgoOrds: [
        {
          clOrdId: 'tp-attach-002',
          tpTriggerPx: '55000',
          slTriggerPx: '48000',
          slOrdPx: '47500',
        } as any,
    ...
    assert.equal(payload2.attachAlgoOrds[0].slOrdPx, '47500');
  });
