 ...
 * （仍不要 import dbService/AccountMonitor 等重链——会拉起常驻连接、拖慢用例，且需靠 tests/env.ts 隔离。
 *   注：better-sqlite3 v13 起为 N-API 预编译，系统 Node 与 Electron 共用同一二进制、ABI 无关，
 *   「为 Electron 编译 / 必须 Electron 环境跑」的旧说法已作废——整套 npm test 在系统 Node 下 240/240 全过。）
 */
