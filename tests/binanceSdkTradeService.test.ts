...
describe("applyTiming", () => {
  ...
  // 时钟只会前进：applyTiming 内部会**重新**读一次 _dateNowPatched()，与测试先取的
  // realNow 之间可能跨毫秒（并发负载下必现）→ 等值断言必然偶发失败。
  // 改用区间断言：下界锁 offset 正确性，上界给时钟前进留容忍窗口。
  ...
  it("验证 timestamp = _dateNowPatched() + offset", () => {
    ...
    svc.applyTiming(payload, false);
    const delta = (payload.timestamp as number) - realNow;
    assert.ok(
      delta >= spotOffset && delta <= spotOffset + TIMING_TOLERANCE_MS,
      `timestamp 偏差应在 [${spotOffset}, ${spotOffset + TIMING_TOLERANCE_MS}]（时钟前进容忍 ${TIMING_TOLERANCE_MS}ms），实际 ${delta}`,
    );
  });
  ...
  it("futures 模式使用 _futuresTimeOffsetMs", () => {
    ...
    const delta = (payload.timestamp as number) - realNow;
    assert.ok(
      delta >= futuresOffset && delta <= futuresOffset + TIMING_TOLERANCE_MS,
      `timestamp 偏差应在 [${futuresOffset}, ${futuresOffset + TIMING_TOLERANCE_MS}]（时钟前进容忍 ${TIMING_TOLERANCE_MS}ms），实际 ${delta}`,
    );
  });
});
