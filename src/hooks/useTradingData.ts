...
import { getApiBase as getApiBaseUrl } from '../lib/apiBase.ts';
...
export function useTradingData() {
  ...
  const bumpHistoryTargetRef = useRef<number | null>(null);
  const bumpHistoryVersion = useCallback((accountIdx?: number) => {
    if (typeof accountIdx === 'number') bumpHistoryTargetRef.current = accountIdx;
  ...
  const { data: tickerData } = useQuery({
    ...
    queryFn: async () => {
      return await safeFetch<{ ok: boolean; data: Record<string, { change24h: number; changeToday: number }> }>('/api/market/tickers/all?exchange=okx')
        ?? { ok: false, data: {} as Record<string, { change24h: number; changeToday: number }> };
    },
    staleTime: 5000,
  ...
  useEffect(() => {
      ...
      try {
        const [snapshotRes, ordersRes] = await Promise.all([
          ...
          (async () => {
            ...
            return await safeFetch<OrdersResponse>('/api/orders/pending');
          })(),
        ]);
    ...
    const connectWs = () => {
      ...
      const processMessage = (msg: Record<string, unknown>) => {
        ...
        if (msg.type === 'strategy_toast') {
          window.dispatchEvent(new CustomEvent('strategy-triggered', { detail: { strategyName: msg.strategyName } }));
          return;
        }
            ...
            msg.data.forEach((o: Partial<Order>) => {
              ...
              if (id) {
                ...
                if (ACTIVE_ORDER_STATES.has(state.toLowerCase())) {
                  ...
                  if (next[key]) bumpHistoryVersion(o._account);
                  delete next[key];
                }
              }
            });
        ...
        if (msg.type === 'positions_update' && msg.data) {
          ...
          if (targetAccount !== undefined && isValidAccountIdx(targetAccount)) {
            ...
            const prevAccountKeys = new Set(
              Object.keys(next).filter(k => k.endsWith(`_${targetAccount}`))
            );

            msg.data.forEach((p: Partial<Position>) => {
              const key = (p.posId ? `${p.posId}_${targetAccount}` : `${p.instId}_${p.posSide}_${targetAccount}`);
              prevAccountKeys.delete(key);
              if (isEmptyPosition(p)) {
                if (next[key]) {
                  delete next[key];
                  bumpHistoryVersion(targetAccount as number);
                }
              } else {
