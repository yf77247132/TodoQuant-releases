...
export function useConfigManager<T extends BaseConfigItem>(
  ...
  const handleStart = useCallback(async (configId: string): Promise<boolean> => {
    const t0 = Date.now();
    console.info(`[start-perf] [t+0ms] 点启动按钮 configId=${configId}`);
    const previousConfigs = queryClient.getQueryData<T[]>([moduleName, 'configs']);
    if (previousConfigs) {
    ...
    try {
      ...
      console.info(`[start-perf] [t+${Date.now() - t0}ms] fetch 返回 (fetch耗时=${Date.now() - tFetch0}ms)`);
      const data = await res.json();
      console.info(`[start-perf] [t+${Date.now() - t0}ms] res.json 完成`);
      ...
      if (!data.ok) {
        ...
        console.info(`[start-perf] [t+${Date.now() - t0}ms] UI 更新完成(失败) 总耗时=${Date.now() - t0}ms`);
        return false;
      } else {
        ...
        console.info(`[start-perf] [t+${Date.now() - t0}ms] UI 更新完成 总耗时=${Date.now() - t0}ms`);
        return true;
      }
    } catch (err: unknown) {
      ...
      console.info(`[start-perf] [t+${Date.now() - t0}ms] UI 更新完成(异常 ${errorMsg}) 总耗时=${Date.now() - t0}ms`);
      return false;
    }
