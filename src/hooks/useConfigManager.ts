...
export function useConfigManager<T extends BaseConfigItem>(
  ...
  const handlePin = useCallback(async (id: string) => {
    const cached = queryClient.getQueryData<T[]>([moduleName, 'configs']);
    if (!cached) return;
    const previousConfigs = [...cached];
    ...
    try {
      ...
      queryClient.setQueryData<T[]>([moduleName, 'configs'], cached);
  ...
  return useMemo(() => ({
    ...
    configs, isLoading, runningMap, hasAnyRunning, selectedConfigId, setSelectedConfigId,
    getFilteredLogs, localToast, fetchConfigs, handleSave, handleDelete, handlePin, handleDuplicate,
    handleStart, handleStop, batchMode, selectedIds, toggleSelect, selectAll, deselectAll,
    handleBatchStart, handleBatchStop, showToast,
  ]);
}
