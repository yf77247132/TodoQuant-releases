...
import { recordEventLoopBlock } from "../services/diagnosticsService.ts";
...
export async function startServer() {
...

  {
    ...
    setInterval(() => {
      const now = Date.now();
      const delay = now - lastCheck - 2000;
      ...
      if (delay > 2000) {
        ...
        if (cpuMs >= delay * 0.5) {
          console.warn(`[EVENT-LOOP] 事件循环阻塞 ${delay}ms（CPU ${cpuMs.toFixed(0)}ms，${new Date().toLocaleTimeString('zh-CN', { hour12: false })}）`);
          recordEventLoopBlock(delay);
        }
      }
    }, 2000);
  }
...
async function shutdown(): Promise<void> {
  ...
  try {
    await ftProcessManager.stopAll();
  } catch {
  }
  process.exit(0);
}
