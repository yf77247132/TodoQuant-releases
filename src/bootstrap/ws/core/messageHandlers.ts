  ...
  if ((channel === 'orders' || channel === 'orders-algo' || channel === 'algo-advance') && message.data) {
    ...
    message.data = message.data.map((d: WSDataItem & Record<string, unknown>) => {
      ...
      if (ordersCache) {
        ...
        if (isAlgoEffective) {
          const oldAlgoId = String(transformed.algoId);
          const newOrdId = transformed.ordId || '';
          LogService.info('ws', `[WS] algo→limit: 算法单触发完成 algoId=${oldAlgoId} newOrdId=${newOrdId} instId=${transformed.instId || ''} 旧记录已移除缓存`);
          ordersCache.removeOrder(accountIdx, String(transformed.algoId));
        } else if (isLive) {
          if (!isAlgoOrderType(transformed) && transformed.algoId) {
            const associatedAlgoId = String(transformed.algoId);
            const ownOrdId = transformed.ordId || '';
            LogService.info('ws', `[WS] algo→limit: 新限价单 ordId=${ownOrdId} 关联旧algoId=${associatedAlgoId} instId=${transformed.instId || ''} 已写入缓存`);
          }
