import { requireAccountMonitor, requireTransformOrder, requireDiyEngine } from '../../../lib/wsServiceRegistry.ts';
...
export const handleDataMessageCore = (
  ...
  if (channel === 'positions' && message.data) {
    requireAccountMonitor().updatePositionsFromWs(accountIdx, message.data as WSDataItem[]);
  }

  if (channel === 'account' && message.data) {
    requireAccountMonitor().updateBalancesFromWs(accountIdx, message.data);
  }

  if (message.data) {
    try {
      requireDiyEngine().onDataPush(accountIdx, channel, message.data);
    } catch {
    }
  }
  ...
  if ((channel === 'orders' || channel === 'orders-algo' || channel === 'algo-advance') && message.data) {
    ...
    message.data = message.data.map((d: WSDataItem & Record<string, unknown>) => {
      ...
      const transformed = requireTransformOrder()(d);
      transformedOrders.push(transformed);
      if (ordersCache) {
        const state = transformed.state || '';
        const isLive = LIVE_STATES.has(state.toLowerCase());
    ...
    requireAccountMonitor().updateOrdersFromWs(accountIdx, transformedOrders);
