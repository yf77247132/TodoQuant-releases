...
export function resolveAccountNameById(
  accountId: string,
  accounts?: { id: string; name: string }[] | null,
  accountNames?: Record<string, string> | Record<number, string> | null
): string | null {
  const aid = String(accountId || '').trim();
  if (!aid) return null;
  const acc = accounts?.find(a => a.id === aid);
  if (acc) return acc.name;
  return (accountNames as Record<string, string> | undefined)?.[aid] ?? null;
}
...
export function getAccountNameFromConfig(
  ...
  const name = resolveAccountNameById(accountId, accounts, accountNames);
  return name ?? tr('account.deleted', '账户(已删除)');
}
