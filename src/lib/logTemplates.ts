    ...
    .map(([val, zhLabel, enLabel]) => {
      ...
      if (zhLabel === '回调比例' || enLabel === 'Callback Ratio') {
        ...
        if (!isNaN(numVal) && numVal >= 0 && numVal <= 1) {
          const pct = numVal * 100;
          const twoDecimals = pct.toFixed(2).replace(/\.?0+$/, '');
          displayVal = `${twoDecimals === '0' && pct > 0 ? pct.toFixed(4).replace(/\.?0+$/, '') : twoDecimals}%`;
        }
      }
...
function translateCondEvalDetail(detail: string, lang: 'zh' | 'en'): string {
  ...
  if (lang === 'en') {
    ...
    result = result.replace(/最低(-?\d)/g, 'Min $1');
...
export const LOG_TEMPLATES: Record<string, {
  ...
  'ws.session.error': {
    zh: (p: { wsType: string; code: number; msg: string }) =>
      `WS ${p.wsType} 会话错误 (${p.code}: ${p.msg || WSErrorZh[p.code] || '未知错误'})，不影响连接状态`,
    en: (p) => `WS ${p.wsType} session error (${p.code}: ${p.msg || WSErrorEn[p.code] || 'Unknown error'}), connection state unchanged`,
  },
  'ws.login.retry': {
  ...
  'diy.cond.accountMissing': {
    zh: (p: { i: number; accountId: string }) =>
      `条件 ${p.i} 指定的账户不存在（ID=${p.accountId}），该条件判为未满足 —— 请检查该条件配置的账户是否已被删除或重建`,
    en: (p: { i: number; accountId: string }) =>
      `Condition ${p.i} references a missing account (ID=${p.accountId}); treated as NOT matched — check whether that account was deleted or recreated`,
  },
  'diy.dateChanged': {
  ...
  'binance.futures.attachAlgoIgnored': {
    zh: (p: { symbol: string }) => `[BINANCE] 合约 ${p.symbol} 常规单不支持附带止盈止损，已忽略`,
    en: (p) => `[BINANCE] Futures ${p.symbol} standard orders do not support attached TP/SL, ignored`,
  },
  'account.auth.failed': {
    category: 'API',
  ...
  'ft.stopDuringStart': {
    zh: (p: { id: string }) => `策略 ${p.id} 正在启动中，已标记取消（启动完成前会被拦截）`,
    en: (p: { id: string }) => `Strategy ${p.id} is starting; stop marked and will be intercepted before startup completes`,
  },
  'ft.dbCleaned': {
  ...
  'routeError': {
    category: 'SYSTEM',
    zh: (p: { route: string; msg: string }) => `路由处理异常 ${p.route}: ${p.msg}`,
    en: (p: { route: string; msg: string }) => `Route error ${p.route}: ${p.msg}`,
  },
  'pin.restartFailed': {
    category: 'SYSTEM',
    zh: (p: { msg: string }) => `账户重排后重建监控失败: ${p.msg}`,
    en: (p: { msg: string }) => `Failed to rebuild monitoring after account reorder: ${p.msg}`,
  },
  'account.balance.unrecognized': {
    category: 'SYSTEM',
    zh: (p: { accountIdx: number; type: string; keys: unknown }) => {
      const ks = Array.isArray(p.keys) ? p.keys.join(', ') : String(p.keys ?? '');
      return `账户#${p.accountIdx} 余额数据结构无法识别（type=${p.type}${ks ? `, keys=${ks}` : ''}）`;
    },
    en: (p: { accountIdx: number; type: string; keys: unknown }) => {
      const ks = Array.isArray(p.keys) ? p.keys.join(', ') : String(p.keys ?? '');
      return `Account #${p.accountIdx} balance payload unrecognized (type=${p.type}${ks ? `, keys=${ks}` : ''})`;
    },
  },
  'account.sync.positions.silentSkip.notMonitored': {
    zh: (p: { accountIdx: number; activeCount: number }) =>
      `账户#${p.accountIdx} 不在监控列表（当前监控 ${p.activeCount} 个），跳过持仓同步`,
    en: (p: { accountIdx: number; activeCount: number }) =>
      `Account #${p.accountIdx} not monitored (${p.activeCount} active), position sync skipped`,
  },
  'account.sync.positions.skip.uninitialized': {
    zh: (p: { accountIdx: number; label: string }) => `${p.label} 账户数据未初始化，跳过持仓同步`,
    en: (p: { accountIdx: number; label: string }) => `${p.label} account data not initialized, position sync skipped`,
  },
  'savingsPoller.pollFailed': {
    category: 'SYSTEM',
    zh: (p: { accountIdx: number; msg: string }) => `账户#${p.accountIdx} 资产估值轮询失败: ${p.msg}`,
    en: (p: { accountIdx: number; msg: string }) => `Account #${p.accountIdx} asset valuation polling failed: ${p.msg}`,
  },
  'system.shutdown.ft': {
    zh: () => '系统关机：停止 freqtrade 进程',
    en: () => 'System shutdown: stopping freqtrade process',
  },
  'system.ws.bufferOverflow': {
    zh: (p: { dropped: number }) => `前端 WS 发送缓冲积压，已丢弃 ${p.dropped} 条消息`,
    en: (p: { dropped: number }) => `Frontend WS send buffer overflow, dropped ${p.dropped} message(s)`,
  },
  'ft.portReleased': {
    zh: (p: { port: number; pid: string }) => `已释放残留端口 ${p.port}（pid ${p.pid}）`,
    en: (p: { port: number; pid: string }) => `Released stale port ${p.port} (pid ${p.pid})`,
  },
  'cancel.refreshFailed': {
    zh: (p: { msg: string }) => `撤单后刷新订单失败: ${p.msg}`,
    en: (p: { msg: string }) => `Order refresh after cancel failed: ${p.msg}`,
  },
  'close.sync.failed': {
    zh: (p: { msg: string }) => `平仓后同步仓位失败: ${p.msg}`,
    en: (p: { msg: string }) => `Position sync after close failed: ${p.msg}`,
  },
  'margin.transfer.insufficient': {
    zh: (p: { amt: number }) => `可用余额不足，本次补仓划转 ${p.amt} USDT 未执行`,
    en: (p: { amt: number }) => `Insufficient available balance, margin top-up transfer of ${p.amt} USDT skipped`,
  },
  'diy.configLoadFailed': {
    category: 'SYSTEM',
    zh: (p: { msg: string }) => `DIY 条件配置加载失败: ${p.msg}`,
    en: (p: { msg: string }) => `DIY condition config load failed: ${p.msg}`,
  },
  'trader.warn.formulaError': {
    zh: (p: { formula: string; error: string }) => `价格公式求值失败 [${p.formula}]: ${p.error}`,
    en: (p: { formula: string; error: string }) => `Price formula evaluation failed [${p.formula}]: ${p.error}`,
  },
  'task.run.rejected.busy': {
    zh: () => '极速下单正在执行中，本次请求已拒绝（防重复下单）',
    en: () => 'Quick trade in progress, request rejected (duplicate order guard)',
  },
  'webhook.rejected.remote': {
    category: 'SYSTEM',
    zh: (p: { addr: string }) => `freqtrade webhook 拒绝非本机请求（来源 ${p.addr}）`,
    en: (p: { addr: string }) => `freqtrade webhook rejected non-local request (from ${p.addr})`,
  },
