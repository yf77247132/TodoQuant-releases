...
import { fetchMarketPrice, fetchSodUtc8 } from './marketPrice.ts';
...
export async function buildFormulaVariables(
  ...
  if (formulaNeedsVar(fml, 'S8') && !variables.S8) {
    if (!params.instId) {
      return { variables, errorCode: 'MISSING_INST_ID' };
    }
    const sod = await fetchSodUtc8(params.instId);
    if (sod === null) {
      return { variables, errorCode: 'S8_UNAVAILABLE' };
    }
    variables.S8 = sod;
  }
...
function getDecimalPlaces(n: number): number {
  ...
  if (str.includes('e') || str.includes('E')) {
    try {
      ...
      const dotIdx2 = fixed.indexOf('.');
      if (dotIdx2 < 0) return 0;
      const trimmed = fixed.replace(/0+$/, '');
      return trimmed.length - dotIdx2 - 1;
    } catch {
      return 8;
    }
  }
