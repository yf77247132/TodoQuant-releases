...
const LEGACY_MIN_DECRYPT_KEY_LENGTH = 6;
...
export function encrypt(text: string, masterKey: string): string {
  ...
  if (!masterKey || masterKey.length < MIN_ENCRYPT_KEY_LENGTH) {
    throw new Error('Master encryption key must be at least 16 characters long.');
  }
...
export function decrypt(encryptedData: string, masterKey: string): string {
  ...
  if (!masterKey || masterKey.length < LEGACY_MIN_DECRYPT_KEY_LENGTH) {
