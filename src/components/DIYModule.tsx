...
import { Cpu, Plus, Activity, List, ListChecks, LineChart } from 'lucide-react';
...
export default React.memo(function DIYModule({ config, logs, onClearLogs, serverTimezone: propServerTimezone, preFillTab, onClearPreFillTab, showToast: onShowToast, isActive = true }: DIYModuleProps) {
  ...
  const placeManager = useConfigManager<PlaceConfigItem>({ moduleName: 'place', apiPrefix: '/api/trader', pollInterval: 30000, toRequestBody: (f) => f });
  const cancelManager = useConfigManager<CancelConfigItem>({ moduleName: 'cancel', apiPrefix: '/api/cancel', pollInterval: 30000, toRequestBody: (f) => f });
  const amendManager = useConfigManager<AmendConfigItem>({ moduleName: 'amend', apiPrefix: '/api/amend', pollInterval: 30000, toRequestBody: (f) => f });
  const closeManager = useConfigManager<CloseConfigItem>({ moduleName: 'close', apiPrefix: '/api/close', pollInterval: 30000, toRequestBody: (f) => f });
  const marginManager = useConfigManager<MarginConfigItem>({ moduleName: 'margin', apiPrefix: '/api/margin', pollInterval: 30000, toRequestBody: (f) => f });
  ...
  const handleStartStrategy = useCallback(async (id: string) => {
    ...
    let liveSpec = strategy?.freqtradeSpec;
