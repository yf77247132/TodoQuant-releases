...
import { PositionTable, filterPositions, type PositionFilters } from './dashboard/PositionTable.tsx';
...
export const PositionsDashboard: React.FC<PositionsDashboardProps> = memo(({
  ...
  const filteredTotalUpl = useMemo(
    () => filterPositions(filteredPos, filters, accountNames, accountIdNames, (key) => t(key))
      .reduce((s, p) => s + (parseFloat(p.upl) || 0), 0),
    [filteredPos, filters, accountNames, accountIdNames, t]
  );

  const filteredTotalMargin = useMemo(
    () => filterPositions(filteredPos, filters, accountNames, accountIdNames, (key) => t(key))
      .reduce((s, p) => s + (parseFloat(String(p.mgnMode === 'cross' ? p.imr : p.margin)) || 0), 0),
    [filteredPos, filters, accountNames, accountIdNames, t]
  );
  ...
  return (
    <div className="flex flex-col space-y-6 animate-in fade-in duration-500 pb-8">
      <div className="flex flex-wrap gap-4 shrink-0">
        <div className="flex-1 min-w-[180px] bg-surface-2 border border-border-default rounded-2xl p-4 flex flex-col shadow-sm">
        ...
        <StatCard className="flex-1 min-w-[180px]" label={t('dashboard.stats.totalUpl')} value={privacyMode ? '****' : `${filteredTotalUpl >= 0 ? '+' : ''}${fmt(filteredTotalUpl, 2)}`} color={filteredTotalUpl > 0 ? 'text-trade-green' : filteredTotalUpl < 0 ? 'text-trade-red' : ''} />
        <StatCard className="flex-1 min-w-[180px]" label={t('dashboard.stats.totalMargin')} value={privacyMode ? '****' : fmt(filteredTotalMargin, 2)} />
        <StatCard className="flex-1 min-w-[180px]" label={t('dashboard.stats.liqWarning')} value={riskyCount} color={riskyCount > 0 ? 'text-trade-red' : ''} />
      </div>
      ...
      <div className="flex-1 flex flex-col gap-6 xl:min-h-0">
        ...
        <div className="flex flex-col space-y-4 shrink-0">
          <ConfigHeader
            ...
            subtitle={(visibleCounts.positions ?? filteredPos.length).toString()}
            actions={
        ...
        <div className="flex flex-col space-y-4">
          <ConfigHeader
            ...
            subtitle={(visibleCounts.orders ?? ordList.length).toString()}
            actions={
              <div className="flex items-center gap-2">
