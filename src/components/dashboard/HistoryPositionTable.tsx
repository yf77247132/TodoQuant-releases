...
const HistoryPositionRow = memo(({ p, accLabel, color, onAccountFilter, onInstIdFilter, onDirectionFilter, onTypeFilter, dataIdx }: HistoryPositionRowProps) => {
  ...
  const isPartialClose = String(p.type) === '1';
  ...
  return (
    <tr className="h-9 hover:bg-surface-3/50 transition-colors" data-idx={dataIdx} data-dir={p.direction}>
      ...
      <td className="px-3 py-2.5 text-left font-mono text-text-secondary">
        {showCloseInfo ? fmtHoldDuration(parseInt(String(p.uTime)) - parseInt(String(p.cTime))) : '-'}
      </td>
      <td className="px-3 py-2.5 text-left">
        {showCloseInfo ? (
          <Tooltip content={t('order.clickToFilter')}>
            <DashedHint className={`cursor-pointer ${isPartialClose ? 'text-text-muted' : 'text-text-secondary'}`}
...
export const HistoryPositionTable = memo(({ positions, accountNames, accountColors, accountIdNames, accountIdColors, hasMore, loadingMore, onLoadMore }: HistoryPositionTableProps) => {
  const { t } = useTranslation();
  const scrollContainerRef = useRef<HTMLDivElement | null>(null);
  useScrollChaining(scrollContainerRef);

  const [renderCount, setRenderCount] = useState(INITIAL_RENDER_COUNT);
  const renderCountRef = useRef(INITIAL_RENDER_COUNT);
  const hasMoreRef = useRef(!!hasMore);
  const loadingMoreRef = useRef(!!loadingMore);
  const onLoadMoreRef = useRef(onLoadMore);
  const setRenderCountBoth = useCallback((next: number) => {
    renderCountRef.current = next;
    setRenderCount(next);
  }, []);
  ...
  const optionCounts = useMemo(() => {
    ...
    positions.forEach(p => {
      ...
      if (p.typeDisplay) type[p.typeDisplay] = (type[p.typeDisplay] || 0) + 1;
    });
  ...
  useEffect(() => {
    setRenderCountBoth(INITIAL_RENDER_COUNT);
  }, [filters.account, filters.instId, filters.direction, filters.type, setRenderCountBoth]);
  ...
  const handleScroll = useCallback(() => {
    ...
    if (renderCountRef.current < sorted.length) {
      setRenderCountBoth(Math.min(renderCountRef.current + LOAD_MORE_STEP, sorted.length));
      return;
    }

    if (hasMoreRef.current && !loadingMoreRef.current) {
      const now = Date.now();
      if (now - lastAutoLoadRef.current >= LOAD_COOLDOWN) {
        lastAutoLoadRef.current = now;
        onLoadMoreRef.current?.();
      }
    }
  }, [sorted.length, setRenderCountBoth]);

  useEffect(() => {
    if (loadingMoreRef.current) return;
    if (sorted.length > 0 && renderCount < sorted.length) {
      setRenderCountBoth(Math.min(renderCount + LOAD_MORE_STEP, sorted.length));
    }
  }, [renderCount, sorted.length, loadingMore, setRenderCountBoth]);
        ...
        <table className="w-full text-left text-xs whitespace-nowrap [&_thead>tr]:!h-9 [&_tbody>tr]:!h-9 [&_tbody>tr>td]:!py-[5px] [&_th]:!py-1.5 [&_td]:leading-[17px] [&_th]:leading-[17px]">
          <thead className="bg-surface-2 text-text-tertiary sticky top-0 z-[var(--z-dropdown)]">
            <tr>
              ...
              <th className="px-3 py-2.5 font-medium text-text-tertiary text-left">
                <Tooltip content={t('position.holdTimeTipHist')}>
          ...
          <tbody key={`filter-${filters.account || 'a'}-${filters.instId || 'i'}-${filters.direction || 'd'}-${filters.type || 't'}`} className="divide-y divide-subtle" data-filter-dir={filters.direction || 'all'} data-visible-count={visiblePositions.length}>
            {visiblePositions.map((p, idx) => {
              ...
              return (
                <HistoryPositionRow
                  key={`${p._accountId}-${p.posId ?? `np:${p.instId}:${p.type ?? ''}:${p.cTime ?? ''}:${p.closeAvgPx ?? ''}`}-${p.uTime || p.cTime || idx}`}
                  p={p}
                  accLabel={accLabel}
                  color={color}
          ...
          <div className="py-4 text-center text-text-tertiary text-xs">
            {loadingMore ? (
              <span className="inline-flex items-center gap-2">
                <Spinner size={14} /> {t('order.loadingMore')}
              </span>
            ) : (
              t('order.scrollForMore')
            )}
          </div>
        )}
      </div>
