...
export const FilterDropdown = memo<FilterDropdownProps>(({
  ...
  <div className="relative inline-block group">
    <button
      ...
      className={`flex items-center gap-1 hover:text-text-primary transition-colors py-0.5 ${filters[field] ? 'text-brand-yellow' : ''}`}
    >
      {labelEl}
    ...
    {activeFilter === field && createPortal(
      <div
        ...
        onMouseDown={(e) => { if (e.target === e.currentTarget) onSelectFilter('', null); }}
      >
        <div
          style={{
            position: 'fixed',
          ...
          className="mt-1 w-44 bg-dropdown-bg border border-border-default rounded-md shadow-2xl py-1 flex flex-col max-h-[360px] overflow-y-auto custom-scrollbar"
        >
          <button
            onClick={() => onSelectFilter(field, null)}
            className="w-full flex items-center justify-between gap-3 px-3 py-1.5 hover:bg-white/5 text-2xs text-text-secondary transition-colors"
          >
            <span className="truncate">{t('dashboard.filter.all')}</span>
            {totalCount != null && <span className="text-text-tertiary tabular-nums shrink-0">{totalCount}</span>}
          </button>
          {sortedOptions.map((opt) => (
            <button
              key={opt}
              onClick={() => onSelectFilter(field, opt)}
              className={`w-full flex items-center justify-between gap-3 px-3 py-1.5 hover:bg-white/5 text-2xs transition-colors ${filters[field] === opt ? 'text-brand-yellow font-bold bg-brand-yellow/5' : 'text-text-primary'}`}
            >
              <span className="truncate">{formatOption ? formatOption(opt) : opt}</span>
              {counts && counts[opt] != null && <span className="text-text-tertiary tabular-nums shrink-0">{counts[opt]}</span>}
            </button>
          ))}
        </div>
      </div>,
...
export const fmtTickSz = (n: string | number, tickSz?: string | number | null): string => {
  ...
  if (tickSz !== undefined && tickSz !== null) {
    ...
    const scientificMatch = tickStr.match(/e-(\d+)$/);
    if (scientificMatch) {
      digits = parseInt(scientificMatch[1], 10);
...
export const fmtPct = (n: string | number): string => {
  ...
  if (isNaN(v)) return '--';
  const truncated = Math.trunc(v * 10000) / 100;
