...
import { Terminal, X, Activity, AlertTriangle, Copy, Check, Server, Globe, Keyboard, Gauge } from 'lucide-react';
...
type TabType = 'logs' | 'errors' | 'diagnostics';
...
interface DiagnosticsSnapshot {
  activeCount: number;
  poolNearFull: boolean;
  poolCapacity: number;
  activeRequests: ActiveRequest[];
  slowRequests: SlowRequest[];
  eventLoopBlockHistory: EventLoopBlock[];
...
const fmtTime = (ts: number, timezone?: string) => {
  ...
  if (timezone) {
    try {
      ...
      console.warn('LogModal timezone format error:', e instanceof Error ? e.message : e);
    }
  }
...
const SystemLogLine = memo(({ l, timezone, accountNames, accountColors }: { l: LogEntry, timezone?: string, accountNames: Record<string, string>, accountColors: Record<string, string> }) => {
  ...
  const cleanMsg = useMemo(() => {
    ...
    if (l.logKey && LOG_TEMPLATES[l.logKey]) {
      ...
      if (isEnglish && template.en) {
        return template.en(l.logParams);
      }
      return rawClean;
    }
  ...
  const accountNameReplacements = useMemo(() => {
    return Object.entries(accountNames)
      .filter(([, name]) => !!name)
      .map(([idx, name]) => ({
        patterns: [new RegExp(`账户\\s*${idx}`, 'g'), new RegExp(`\\[Account ${idx}\\]`, 'g')],
        name: name as string,
      }));
  }, [accountNames]);

  let processedMsg = cleanMsg;
  accountNameReplacements.forEach(({ patterns, name }) => {
    patterns.forEach(re => {
      processedMsg = processedMsg.replace(re, name);
    });
  });
...
export const LogModal: React.FC<LogModalProps> = React.memo(({ isOpen, onClose, logs, envStatus, onClear, timezone, accountNames: accountNamesProp, accountColors: accountColorsProp }) => {
  ...
  const diagnosticsQuery = useQuery<DiagnosticsSnapshot>({
    queryKey: ['diagnostics'],
    queryFn: async () => {
      const res = await fetch('/api/diagnostics');
      const data = await res.json();
      return (data?.data ?? null) as DiagnosticsSnapshot;
    },
    refetchInterval: 5000,
    refetchOnWindowFocus: false,
    enabled: isOpen && activeTab === 'diagnostics',
  });
  const diag = diagnosticsQuery.data;
  
  const accountNames = accountNamesProp || {};
  const accountColors = accountColorsProp || {};
    ...
    const logText = logs
      ...
      .map(log => {
        ...
        if (log.logKey && LOG_TEMPLATES[log.logKey]) {
          ...
          if (isEnglish && template.en) {
            translatedMsg = template.en(log.logParams);
          } else {
        ...
        <div className="px-6 py-5 border-b border-border-default bg-surface-2 shrink-0">
          ...
          <div className="flex gap-1.5 p-1 bg-surface-1 rounded-xl w-fit border border-border-default">
            ...
            <TabButton
              ...
              icon={Gauge}
              label={t('logModal.diagnostics')}
              onClick={() => setActiveTab('diagnostics')}
              badge={diag?.poolNearFull ? '!' : undefined}
            />
          </div>
        </div>
                            ...
                            <p className={`text-sm font-medium ${textColor}`}>
                              {stat.logKey && LOG_TEMPLATES[stat.logKey]
                                ? (i18n.language?.startsWith('en') && LOG_TEMPLATES[stat.logKey].en
                                    ? LOG_TEMPLATES[stat.logKey].en(stat.logParams)
          ...
          {activeTab === 'diagnostics' && (
            <div className="space-y-4">
              <div className={`p-4 rounded-xl border ${
                diag?.poolNearFull ? 'border-trade-red/40 bg-trade-red/5' : 'border-border-default bg-surface-2'
              }`}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Gauge className={`w-4 h-4 ${diag?.poolNearFull ? 'text-trade-red' : 'text-brand-blue'}`} />
                    <span className="text-xs font-bold uppercase tracking-widest text-text-tertiary">
                      {t('logModal.connectionPool')}
                    </span>
                  </div>
                  <div className={`text-2xl font-bold ${diag?.poolNearFull ? 'text-trade-red' : 'text-text-primary'}`}>
                    {diag?.activeCount ?? 0}
                    <span className="text-text-tertiary text-sm font-medium"> / {diag?.poolCapacity ?? 6}</span>
                  </div>
                </div>
                <div className="mt-2 h-2 rounded-full bg-surface-3 overflow-hidden">
                  <div
                    className={`h-full transition-all ${diag?.poolNearFull ? 'bg-trade-red' : 'bg-brand-blue'}`}
                    style={{ width: `${Math.min(100, ((diag?.activeCount ?? 0) / (diag?.poolCapacity ?? 6)) * 100)}%` }}
                  />
                </div>
                {diag?.poolNearFull && (
                  <p className="mt-2 text-2xs text-trade-red">
                    {t('logModal.poolNearFullWarn')}
                  </p>
                )}
              </div>

              <div>
                <h3 className="text-xs font-bold text-text-tertiary uppercase tracking-widest mb-2">
                  {t('logModal.activeRequests')} <span className="text-text-primary ml-1">({diag?.activeRequests.length ?? 0})</span>
                </h3>
                {(!diag || diag.activeRequests.length === 0) ? (
                  <div className="py-8 flex flex-col items-center justify-center text-text-muted italic">
                    <Check className="w-10 h-10 mb-3 opacity-5" />
                    <p className="text-2xs tracking-widest">{t('logModal.noActiveRequests')}</p>
                  </div>
                ) : (
                  <div className="space-y-1">
                    {diag.activeRequests.map(r => (
                      <div
                        key={r.id}
                        className={`flex items-center justify-between px-3 py-2 rounded-lg border ${
                          r.durationMs > 3000 ? 'border-trade-red/30 bg-trade-red/5' : 'border-border-default bg-surface-2'
                        }`}
                      >
                        <div className="flex items-center gap-2 min-w-0 flex-1">
                          <span className={`text-2xs font-bold px-1.5 py-0.5 rounded ${
                            r.method === 'GET' ? 'bg-brand-blue/20 text-brand-blue' : 'bg-brand-yellow/20 text-brand-yellow'
                          }`}>{r.method}</span>
                          <span className="font-mono text-2xs truncate text-text-text">{r.path}</span>
                          {r.query && <span className="text-2xs text-text-tertiary truncate">?{r.query}</span>}
                        </div>
                        <span className={`text-2xs font-bold ml-2 shrink-0 ${
                          r.durationMs > 3000 ? 'text-trade-red' : r.durationMs > 1000 ? 'text-brand-yellow' : 'text-text-tertiary'
                        }`}>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div>
                  <h3 className="text-xs font-bold text-text-tertiary uppercase tracking-widest mb-2">
                    {t('logModal.slowRequests')} <span className="text-text-primary ml-1">({diag?.slowRequests.length ?? 0})</span>
                  </h3>
                  {(!diag || diag.slowRequests.length === 0) ? (
                    <div className="py-6 text-center text-text-muted italic text-2xs">{t('logModal.noSlowRequests')}</div>
                  ) : (
                    <div className="space-y-1 max-h-60 overflow-y-auto custom-scrollbar">
                      {diag.slowRequests.map((r, i) => (
                        <div key={i} className="flex items-center justify-between px-3 py-1.5 rounded-lg border border-border-default bg-surface-2 text-2xs">
                          <span className="font-mono truncate flex-1">{r.method} {r.path}</span>
                          <span className="font-bold text-trade-red ml-2 shrink-0">{(r.durationMs / 1000).toFixed(1)}s</span>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div>
                  <h3 className="text-xs font-bold text-text-tertiary uppercase tracking-widest mb-2">
                    {t('logModal.eventLoopBlocks')} <span className="text-text-primary ml-1">({diag?.eventLoopBlockHistory.length ?? 0})</span>
                  </h3>
                  {(!diag || diag.eventLoopBlockHistory.length === 0) ? (
                    <div className="py-6 text-center text-text-muted italic text-2xs">{t('logModal.noEventLoopBlocks')}</div>
                  ) : (
                    <div className="space-y-1 max-h-60 overflow-y-auto custom-scrollbar">
                      {diag.eventLoopBlockHistory.map((b, i) => (
                        <div key={i} className="flex items-center justify-between px-3 py-1.5 rounded-lg border border-border-default bg-surface-2 text-2xs">
                          <span className="font-mono text-text-tertiary">
                            {new Date(b.timestamp).toLocaleTimeString('zh-CN', { hour12: false })}
                          </span>
                          <span className={`font-bold ml-2 shrink-0 ${b.delayMs > 2000 ? 'text-trade-red' : 'text-brand-yellow'}`}>
                            {b.delayMs}ms
                          </span>
                        </div>
                      ))}
