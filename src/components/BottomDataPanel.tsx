...
import { ShieldCheck, Wallet, FileText, Clock } from 'lucide-react';
...
export const BottomDataPanel: React.FC<BottomDataPanelProps> = memo(({
  ...
  useEffect(() => {
    if (historyOrders.length === 0 && !historyLoading) {
      const fetchHistoryOrders = async () => {
        setHistoryLoading(true);
        try {
          const allHistory: Order[] = [];
          const accounts = (accList as any[]);

          for (const account of accounts) {
            const accountId = account._accountId || account.id;
            if (!accountId) continue;

            const [normalRes, algoRes] = await Promise.all([
              fetch('/api/orders/history', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ accountId, instType: 'SWAP', limit: 100 }),
              }),
              fetch('/api/orders/algo-history', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ accountId, instType: 'SWAP', limit: 100 }),
              }),
            ...
            if (normalData.ok && Array.isArray(normalData.data)) {
              allHistory.push(...normalData.data);
            }
            if (algoData.ok && Array.isArray(algoData.data)) {
              allHistory.push(...algoData.data);
            }
          }
  ...
  }, [historyOrders.length, historyLoading, accList]);
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          if (data.ok) {
            ...
            showToast(`${t('order.cancelFailed')}：${data.error || t('order.networkError')}`, 'error');
          }
        } catch (e: unknown) {
          const err = e instanceof Error ? e : new Error(String(e));
          ...
          {DATA_TABS.map(tab => {
            ...
            return (
              ...
              >
                ...
                {tab.key === 'history' ? (
                  <Tooltip content={t('dashboard.section.historyOrdersTip')}>
