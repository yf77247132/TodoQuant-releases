  ...
  const handleBatchClose = useCallback(() => {
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        ...
        console.info(`[batch-close] [t+0ms] 用户点击确认批量平仓 共 ${selectedCount} 笔`);
        ...
        try {
          ...
          console.info(`[batch-close] [t+${Date.now() - t0}ms] /api/position/batch-close 返回 fetch耗时=${tFetchBack - tFetch0}ms 成功=${data.success} 失败=${data.fail}`);
          if (data.ok) {
            if (data.fail > 0) {
