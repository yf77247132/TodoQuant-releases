  ...
  return (
    <div className="bg-surface-2 border border-border-default rounded-2xl shadow-sm overflow-hidden flex flex-col h-full">
      <div className="flex items-center gap-2 px-3 py-2 bg-surface-2 border-b border-border-subtle">
        ...
        <div className="relative flex-1 max-w-[220px] min-w-0">
          <input
            type="text"
            value={search}
            onChange={e => setSearch(e.target.value)}
            placeholder={t('monitor.searchPlaceholder')}
            className="w-full pl-2 pr-7 py-1 text-xs bg-surface-1 border border-border-default rounded text-text-primary focus:border-focus-ring focus:outline-none"
          />
          {search && (
            <button
              type="button"
              ...
              aria-label={t('monitor.clearSearch')}
              className="absolute right-1 top-1/2 -translate-y-1/2 p-0.5 rounded text-text-muted hover:text-text-primary hover:bg-surface-2 transition-colors"
            >
              <X className="w-3 h-3" />
            </button>
          )}
