...
import { buildCancelDescSegments } from '../lib/configDesc.ts';
import type { CancelConfigItem } from '../types/configItems.ts';
          ...
          <tbody>
            {orders.map((o, idx) => (
              <tr key={idx} className={`${idx % 2 === 0 ? 'bg-surface-1' : ''} hover:bg-surface-3`}>
                ...
                <td className="py-1 px-1 text-text-primary whitespace-nowrap">
                  <span className="inline-flex items-center gap-1">
                    <CoinIcon symbol={o.instId.split('-')[0]} size={12} />
                    {o.instId}
                  </span>
                </td>
...
export default React.memo(function CancelModule({
  ...
  const fetchPreviewCancel = useCallback(() => {
    ...
    previewTimerRef.current = setTimeout(() => { void doFetchPreview(); }, 300);
  }, [doFetchPreview]);
  ...
  useEffect(() => {
    if (!showModal) return;
    if (isModalJustOpenedRef.current) {
      isModalJustOpenedRef.current = false;
      return;
    }
    fetchPreviewCancel();
  }, [form.account_id, form.inst_id, form.order_types, form.side, showModal, fetchPreviewCancel]);
  ...
  const { handleSaveConfig, handleSaveAndStart } = useMemo(() => createModuleSaveHandlers<CancelFormState>({
    ...
    handleRunMode: async (configData) => {
      window.dispatchEvent(new CustomEvent('open-log-drawer', { detail: { moduleKey: 'cancel', force: true } }));
      closeModal();
      try {
        ...
        const data = await res.json();
        if (!data.ok) {
          onShowToast(t('cancel.ui.statusFailed', { error: data.error || t('cancel.ui.statusDefaultError') }), 'error');
        }
      } catch (e: unknown) {
        const err = e instanceof Error ? e : new Error(String(e));
        console.error('立即执行撤单失败:', err.message);
        onShowToast(t('cancel.ui.statusNetworkError'), 'error');
      }
    },
  }), [handleSave, handleStart, closeModal, fetchConfigs, isRunMode, t, onShowToast]);
  ...
  const configModalProps = {
    open: showModal,
    title: isPrefillMode ? t('order.batchCancel') : (isRunMode ? t('cancel.ui.runTitle') : (editingId ? t('cancel.ui.editTitle') : t('cancel.ui.newTitle'))),
