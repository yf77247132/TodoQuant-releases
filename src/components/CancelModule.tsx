...
export default React.memo(function CancelModule({
  ...
  const { handleSaveConfig, handleSaveAndStart } = useMemo(() => createModuleSaveHandlers<CancelFormState>({
    ...
    handleRunMode: async (configData) => {
      const t0 = Date.now();
      const typesCount = (configData.order_types || []).length;
      console.info(`[cancel-run] [t+0ms] 用户点击立即执行 types=${typesCount} account=${configData.account_id}`);
      try {
        ...
        const tFetchBack = Date.now();
        const data = await res.json();
        console.info(`[cancel-run] [t+${Date.now() - t0}ms] /api/cancel/run 返回 fetch耗时=${tFetchBack - tFetch0}ms ok=${data.ok}`);
        if (data.ok) {
          closeModal();
        ...
        console.info(`[cancel-run] [t+${Date.now() - t0}ms] UI 更新完成 总耗时=${Date.now() - t0}ms`);
      } catch (e: unknown) {
