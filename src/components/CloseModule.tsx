...
import { buildCloseDescSegments } from '../lib/configDesc.ts';
import type { CloseConfigItem } from '../types/configItems.ts';
...
export default memo(function CloseModule({
  ...
  const {
    ...
    toRequestBody: (form) => ({
      ...
      cancel_pending: form.cancel_pending ?? false,
  ...
  const openEdit = (cfg: CloseConfigItem) => {
    ...
    setForm({
      ...
      cancel_pending: cfg.cancel_pending ?? false,
  ...
  const { handleSaveConfig, handleSaveAndStart } = useMemo(() => createModuleSaveHandlers<CloseFormState>({
    ...
    buildConfigData: (form: CloseFormState) => ({
      ...
      cancel_pending: form.cancel_pending,
      shortcut_key: form.shortcut_key || '',
    }),
  ...
  const configModalChildren = (
    <>
      ...
      <Switch
        checked={form.cancel_pending}
        onChange={(v) => setForm(prev => ({ ...prev, cancel_pending: v }))}
        label={t('close.switch.cancelPendingLabel')}
        descriptionOn={t('close.switch.cancelPendingOn')}
        descriptionOff={t('close.switch.cancelPendingOff')}
        colorOn="text-brand-yellow"
      />
    </>
  );
