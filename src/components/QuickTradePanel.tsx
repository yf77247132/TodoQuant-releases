...
export default function QuickTradePanel({
  ...
  const handleFormulaBlur = useCallback(async (field: string, value: string) => {
    ...
    try {
      const res = await fetch(`${await getApiBase()}/api/evaluate-formula`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ formula: value, instId }),
      });
      const data = await res.json();
      if (data.ok && data.preview) {
        setPricePreviews(prev => ({ ...prev, [field]: { text: String(data.preview), error: false } }));
      } else if (data.errorCode) {
        const i18nKey = `formula.error.${data.errorCode}`;
        setPricePreviews(prev => ({ ...prev, [field]: { text: data.error ? t(i18nKey, { var: data.error }) : t(i18nKey), error: true } }));
      } else {
        setPricePreviews(prev => ({ ...prev, [field]: { text: data.error || data.preview || 'Error', error: true } }));
      }
    } catch {
      setPricePreviews(prev => ({ ...prev, [field]: { text: t('formula.error.requestFailed'), error: true } }));
    }
  }, [instId, t]);
  ...
  const handleSubmit = useCallback(async (submitSide: 'buy' | 'sell') => {
    ...
    try {
      ...
      const res = await fetch(`${await getApiBase()}/api/trader/run`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
