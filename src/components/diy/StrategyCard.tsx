...
import type { PlaceConfigItem, CancelConfigItem, AmendConfigItem, CloseConfigItem, MarginConfigItem } from '../../types/configItems.ts';
...
import { useTunnelStatus, updateTunnelState } from './useTunnelStatus.ts';
import { getApiBase } from '../../lib/apiBase.ts';
...
const StrategyCard = memo(({
  ...
  const getLiveSummary = () => {
    ...
    return template.conditions.map((c, idx) => {
      ...
      switch (c.type) {
        ...
        case 'pos_closed_pnl': desc = `[${accountName}]${params.instId || t('strategy.all')} ${t('strategy.closedPnlDesc')}${params.operator}${params.amount} USDT`; break;
        case 'pos_hold_time': desc = `[${accountName}]${params.instId || t('strategy.all')} ${t('strategy.holdTimeDesc')}${params.operator}${fmtHoldTimeThreshold(params.value, params.unit)}`; break;
        case 'time_window':
          const daysStr = formatWeekDays(params.days, t);
        ...
        case 'price_change_low24h': {
          ...
          const pcInstIdRaw = String(params.instId || '');
          const pcInstId = pcInstIdRaw === '@positions' ? t('condition.currentPositions') : pcInstIdRaw;
          const pcWindow = c.type === 'price_change_high24h'
            ? t('condition.typePriceChangeHigh24h')
            : c.type === 'price_change_low24h'
              ? t('condition.typePriceChangeLow24h')
              : c.type === 'price_change_today' || params.window === 'today'
                ? t('condition.windowToday')
                : t('condition.window24h');
  ...
  const { url: tunnelUrl, status: tunnelStatus } = useTunnelStatus();
  const [restarting, setRestarting] = useState(false);

  const handleRestartTunnel = async () => {
    setRestarting(true);
    updateTunnelState({ status: 'starting', url: '' });
    try {
      const base = await getApiBase();
      const port = base ? Number(new URL(base).port) : Number(window.location.port);
      ...
      if (data.ok && data.url) {
        updateTunnelState({ url: data.url, status: 'running' });
      } else {
        updateTunnelState({ status: 'error' });
      }
    } catch {
      updateTunnelState({ status: 'error' });
    } finally {
      ...
      <div className="flex flex-col gap-2">
        <div className="flex flex-wrap items-center gap-1">
          ...
          <div className="px-1.5 py-0.5 rounded-full text-xs flex items-center gap-1 border border-border-default bg-surface-1 w-fit max-w-full">
            ...
            <div className="flex items-center min-w-0">
              <span className={`font-medium ${strategy.auto_start ? 'text-brand-yellow' : 'text-text-tertiary'}`}>
                {strategy.auto_start ? t('strategy.on') : t('strategy.off')}
              </span>
            </div>
          </div>
