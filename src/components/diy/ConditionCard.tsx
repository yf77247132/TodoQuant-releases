...
const ConditionCard = memo(({
  ...
  const resolveAccountDisplay = (params: Record<string, any>): { name: string; color: string } => {
    ...
    const name = accountId
      ? (resolveAccountNameById(accountId, accounts, accountNames) ?? t('account.deleted'))
      : t('account.unselected');
    ...
    >
      <div className="flex flex-wrap items-center gap-1">
        {template.conditions.map((c) => {
            ...
            content = (
              <>
                ...
                <span className="text-text-secondary ml-1">
                  {formatWeekDays(c.params.days, t)}
                </span>
              </>
            );
          ...
          } else if (c.type === 'price_change' || c.type === 'price_change_24h' || c.type === 'price_change_today' || c.type === 'price_change_high24h' || c.type === 'price_change_low24h') {
            const pcExchange = String(c.params.exchange || 'okx').toUpperCase();
            const pcInstIdRaw = String(c.params.instId || '');
