    ...
    switch (c.type) {
      ...
      case 'time_window': {
        ...
        if (!Array.isArray(days) || days.length === 0) {
          summary = `[${typeLabel}]${params.startTime || '?'}-${params.endTime || '?'}`;
        } else {
          const dayNamesStr = t('strategy.dayNames');
          const dayNames = dayNamesStr.split(',').map(s => s.trim());
          const daysStr = days.length === 7
            ? t('strategy.everyDay')
            : `${t('strategy.weekPrefix')}${[...days].sort((a, b) => a - b).map((d: number) => (dayNames[d] || '?')).join('')}`;
