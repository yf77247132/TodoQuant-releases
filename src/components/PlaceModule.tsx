...
import { filterOrderTypesByContext, isRegularOrder, isTrigger, isConditional, isOCO, isChase, isMoveOrderStop } from '../constants/orderTypes.ts';
import { buildPlaceDescSegments } from '../lib/configDesc.ts';
import type { PlaceConfigItem } from '../types/configItems.ts';
...
const OrderPreviewPanel = memo(({ orders, t }: {
  ...
  const fmtTpSl = (trigger: number | string | null | undefined, ord: number | string | null | undefined): string => {
    if (trigger == null || trigger === '') return '-';
    const triggerText = String(trigger);
    if (ord == null || ord === '') return triggerText;
    const ordText = String(ord);
    const tNum = parseFloat(triggerText);
    const oNum = parseFloat(ordText);
    if (Number.isFinite(tNum) && Number.isFinite(oNum) && tNum === oNum) {
      return triggerText;
    }
  ...
  const hasTpPrice = orders.some(o => o.tpPrice != null || o.tpOrdPx != null);
  const hasSlPrice = orders.some(o => o.slPrice != null || o.slOrdPx != null);
          ...
          <tbody>
            {orders.map((o, idx) => (
              <tr key={o.num} className={`${idx % 2 === 0 ? 'bg-surface-1' : ''} hover:bg-white/[0.05]`}>
                ...
                {hasTpPrice && <td className="py-1 px-1 text-right font-mono text-trade-green/90 whitespace-nowrap">{fmtTpSl(o.tpPrice, o.tpOrdPx)}</td>}
                {hasSlPrice && <td className="py-1 px-1 text-right font-mono text-trade-red/90 whitespace-nowrap">{fmtTpSl(o.slPrice, o.slOrdPx)}</td>}
...
export default React.memo(function PlaceModule({
  ...
  const fetchFormulaPreview = useCallback(async (fieldName: string, value: string, silent = false, extraVars?: Record<string, number>) => {
    ...
    try {
      ...
      const needsS8 = formulaNeedsVar(value, 's8');
