...
import { AMEND_ORDER_TYPE_OPTIONS, AMEND_ORDER_TYPE_VALUES, normalizeOrderType } from '../constants/orderTypes.ts';
import { buildAmendDescSegments } from '../lib/configDesc.ts';
import type { AmendConfigItem } from '../types/configItems.ts';
...
export default React.memo(function AmendModule({
  ...
  const { handleSaveConfig, handleSaveAndStart } = useMemo(() => createModuleSaveHandlers<AmendFormState>({
    ...
    handleRunMode: async (configData) => {
      ...
      try {
        ...
        const data = await res.json();
        if (!data.ok) {
          onShowToast(`✗ ${data.error || t('amend.execFailed')}`, 'error');
        }
      } catch (e: unknown) {
        const err = e instanceof Error ? e : new Error(String(e));
        console.error('立即执行改单失败:', err.message);
        onShowToast(t('amend.networkError'), 'error');
      }
    },
  ...
  const configModalProps = {
    open: showModal,
    title: isPrefillMode ? t('order.batchAmend') : (isRunMode ? `⚡ ${t('amend.runBatchAmend')}` : (editingId ? `✏️ ${t('amend.editAmendConfig')}` : `➕ ${t('amend.newAmendConfig')}`)),
