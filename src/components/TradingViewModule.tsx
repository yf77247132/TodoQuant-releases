...
import { Settings, X, List, Trash2, AlertTriangle, ArrowUpToLine, LineChart, Pencil, Check, ChartCandlestick } from 'lucide-react';
...
import { consumePendingTvSymbol, requestTvSymbol } from '../lib/tvSymbolRequest.ts';
...
function TradingViewWidget({ isActive = true, config, updateConfig }: TradingViewModuleProps) {
  ...
  useEffect(() => {
    if (!isActive) return;
    const pending = consumePendingTvSymbol();
    if (pending) {
      setOverrideSymbol(pending);
    ...
    const handler = (e: Event) => {
      ...
      if (symbol) {
        ...
        setRefreshKey(prev => prev + 1);
        consumePendingTvSymbol();
      }
    };
    window.addEventListener('tv-set-symbol', handler);
    return () => window.removeEventListener('tv-set-symbol', handler);
  }, [isActive]);
  ...
  useEffect(
    () => {
      ...
      const widgetConfig: Record<string, unknown> = {
        ...
        "save_image": true,
        "style": "1",
        "symbol": (overrideSymbol || settings.symbol).replace(/\.P$/, 'PERP'),
        "theme": "dark",
  ...
  const [symbolToEdit, setSymbolToEdit] = useState<string | null>(null);
  const [editValue, setEditValue] = useState('');

  const handleStartEdit = (symbol: string) => {
    setSymbolToEdit(symbol);
    setEditValue(symbol);
  };

  const confirmEdit = () => {
    if (symbolToEdit && editValue && editValue !== symbolToEdit && !tempWatchlist.includes(editValue)) {
      setTempWatchlist(tempWatchlist.map(s => s === symbolToEdit ? editValue : s));
    }
  ...
  return (
    <div className="w-full flex flex-col bg-transparent relative h-full">
      ...
      {isWatchlistOpen && createPortal(
        <div ref={watchlistPanelRef} className="fixed right-[496px] top-12 z-[250] w-80 bg-surface-3 border border-border-default rounded-xl shadow-2xl p-4 animate-in fade-in zoom-in duration-200">
                    ...
                    {symbolToEdit === symbol ? (
                      <>
                        <TextInput
                          ...
                          containerClassName=""
                        />
                        <button onClick={confirmEdit} className="text-trade-green hover:text-trade-green/80 px-1" title={t('common.confirm')}>
                          <Check className="w-3.5 h-3.5" />
                        </button>
                        <button onClick={cancelEdit} className="text-text-muted hover:text-text-primary px-1" title={t('common.cancel')}>
                      ...
                      <>
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            type="button"
                            onClick={(e) => { e.stopPropagation(); requestTvSymbol(symbol); }}
                            className="text-text-muted hover:text-trade-green transition-colors"
                            title={t('position.clickToChart')}
                          >
      ...
      {isSettingsOpen && createPortal(
        <div ref={settingsPanelRef} className="fixed right-[496px] top-12 z-[250] w-80 bg-surface-3 border border-border-default rounded-xl shadow-2xl p-4 animate-in fade-in zoom-in duration-200">
          <div className="flex items-center justify-between mb-4">
