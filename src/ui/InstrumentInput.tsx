...
import { getFavoriteInstruments } from '../lib/favoriteInstruments.ts';
...
const favoritesToSuggestions = (exchange: string): Instrument[] =>
  getFavoriteInstruments(exchange).map(f => ({
    instId: f.instId,
    instType: f.instType,
...
export default function InstrumentInput({
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (val) {
      ...
      setSuggestions(favoritesToSuggestions(exchange));
      setShowDropdown(true);
    }
  };
  ...
  const displayValue = extraOptions.find((o) => o.value === localValue)?.label ?? localValue;
  const showIdleState = !localValue || extraOptions.some((o) => o.value === localValue);
    ...
    <>
      <div className={`space-y-2 relative ${className}`} ref={wrapperRef}>
        ...
        <input
          ...
          onFocus={() => {
            const editingSentinel = extraOptions.some((o) => o.value === localValue);
            if (localValue && !editingSentinel) {
              setShowDropdown(true);
      ...
      {showDropdown && (suggestions.length > 0 || extraOptions.length > 0) && inputRef.current && createPortal(
        <ul
          ref={dropdownRef}
        ...
        >
          {showIdleState && extraOptions.map((opt) => (
            <li
              key={opt.value}
              className="px-3 py-2 text-sm text-brand-yellow hover:bg-brand-blue/20 hover:text-brand-yellow cursor-pointer transition-colors border-b border-border-subtle"
          ...
          {suggestions.map((inst) => (
            <li
              key={inst.instId}
              className="px-3 py-2 text-sm text-text-secondary hover:bg-brand-blue/20 hover:text-text-primary cursor-pointer transition-colors"
