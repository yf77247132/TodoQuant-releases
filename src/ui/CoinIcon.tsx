...
const localSvgInflight = new Map<string, Promise<string | null>>();

async function fetchLocalSvg(symbol: string): Promise<string | null> {
  ...
  const promise = (async () => {
    try {
      ...
      localSvgCache.set(symbol, dataUri);
      return dataUri;
    } catch {
      return null;
    }
...
const CoinIcon: React.FC<CoinIconProps> = memo(({ symbol, size = 18, className }) => {
  ...
  const getInitialState = (): { localUri: string | null; localDone: boolean; cdnUrl: string | null; cdnDone: boolean; retryCount: number } => {
    if (!upperSymbol) return { localUri: null, localDone: false, cdnUrl: null, cdnDone: false, retryCount: 0 };
    const localCached = localSvgCache.get(upperSymbol);
    if (localCached !== undefined) {
      if (localCached && localCached.startsWith('data:image')) return { localUri: localCached, localDone: true, cdnUrl: null, cdnDone: false, retryCount: 0 };
      ...
      if (cdnCached && (cdnCached.expireAt === 0 || cdnCached.expireAt > Date.now())) {
        return { localUri: null, localDone: true, cdnUrl: cdnCached.url, cdnDone: true, retryCount: 0 };
      }
      return { localUri: null, localDone: true, cdnUrl: null, cdnDone: false, retryCount: 0 };
    }
    return { localUri: null, localDone: false, cdnUrl: null, cdnDone: false, retryCount: 0 };
  };

  type State = { localUri: string | null; localDone: boolean; cdnUrl: string | null; cdnDone: boolean; retryCount: number };
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    fetchLocalSvg(upperSymbol).then(uri => {
      if (!cancelled) {
        dispatch({ localUri: uri, localDone: true });
      }
    });
