...
import type { ActionPlaceOrderParams } from "../types/blocks.ts";
import type { OKXApiResponse } from "../types/okx.ts";
...
export interface BinanceAssetValuation {
  totalBal: string;
  details: { funding: string; trading: string; earn: string };
}
...
export class BinanceSdkTradeService {
  ...
  lastTimingProbeAt = 0;
   ...
   parsePercentIncrement(rawValue: unknown): { value: number; isPercent: boolean } {
  ...
  get usdsFutures() {
   ...
   isTimestampError(err: unknown): boolean {
  ...
  private static readonly NETWORK_ERROR_DEBOUNCE_MS = 60000;
   ...
   getNetworkErrorType(err: unknown): string {
    const msg = String((err as Error)?.message || '');
   ...
   shouldSkipNetworkLog(err: unknown): boolean {
   ...
   resolveTimeEndpoint(isFutures: boolean): string {
    if (isFutures) {
  ...
  private _futuresLastSyncAt = 0;
   sharedOrderListMap: Record<string, string> | null = null;
   sharedLastOcoFetch = 0;
  ...
  private static _dateNowPatched = (() => {
   ...
   async syncServerTime(isFutures: boolean, force = false): Promise<void> {
  ...
  async syncTime(force = false): Promise<void> {
   ...
   applyTiming(payload: Record<string, unknown>, isFutures: boolean) {
  ...
  private _cachedFuturesAccount: BinanceFuturesAccountInfo | null = null;
  private _lastFuturesFetch = 0;
  private _cachedPositionRisk: BinancePositionRisk[] | null = null;
  ...
  public clearTradeCaches() {
   ...
   async fetchFuturesAccountCached(): Promise<BinanceFuturesAccountInfo | null> {
    const now = Date.now();
   ...
   async fetchPositionRiskCached(): Promise<BinancePositionRisk[] | null> {
    const now = Date.now();
  ...
  async getAccountBalance(ccy: string = "USDT"): Promise<BinanceBalanceSummary | null> {
    try {
      ...
      if (uAcct && Array.isArray(uAcct.assets)) {
        const bal = uAcct.assets.find((b) => b.asset === ccy);
        if (bal) {
  ...
  async getSavingsBalance(ccy: string): Promise<string | null> {
    try {
      ...
      if (typeof e === 'object' && e !== null && 'response' in e) {
        const errorData = (e as { response?: { data?: { msg?: unknown } } }).response?.data;
  ...
  async getAssetValuation(ccy: string = "USDT"): Promise<BinanceAssetValuation | null> {
    try {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- 解包结果结构随端点而异，强制收窄会在十余个调用点级联 unknown 访问
  async extractData(res: unknown): Promise<any> {
    let data;
    if (res && typeof res === 'object' && 'data' in res) {
      const d = (res as { data?: unknown }).data;
      data = typeof d === 'function' ? await d() : d;
    } else {
      data = res || {};
    }
   ...
   patchDateNowForSdk(isFutures: boolean): boolean {
    const offset = isFutures
      ? this._futuresTimeOffsetMs
      : this._spotTimeOffsetMs;
   ...
   async withTimestampRetry<T>(isFutures: boolean, runner: () => Promise<T>, useSdkTimestamp: boolean = true): Promise<T> {
   ...
   async maybeProbeNetworkPhases(isFutures: boolean, reason: string): Promise<void> {
   ...
   async probeHttpsPhases(urlText: string): Promise<{
    ok: boolean;
    statusCode?: number;
    dnsMs?: number;
    tcpMs?: number;
   ...
   toBinanceSymbol(instId: string): string {
   ...
   fromBinanceSymbol(symbol: string): string {
   ...
   isFutures(instId: string, _tdMode?: string): boolean {
    const text = String(instId || "").toUpperCase();
    if (text.endsWith("-SWAP") || text.endsWith("-FUTURES")) return true;
    return false;
  }

   normalizeBinanceFuturesOrdType(rawType: unknown, rawTif: unknown): string {
   ...
   inferConditionalExecutionType(side: string, triggerPrice: number, marketPrice: number): "STOP" | "TAKE_PROFIT" {
   ...
   async getSpotSymbolInfo(symbol: string): Promise<{ pricePrecision: number; quantityPrecision: number; minNotional: number } | null> {
   ...
   async getFuturesSymbolInfo(symbol: string): Promise<{ pricePrecision: number; quantityPrecision: number; minNotional: number } | null> {
   ...
   ok(data?: Array<Record<string, unknown>>) {
    return {
      code: "0",
      msg: "",
   ...
   roundStep(value: number, precision: number): string {
  ...
  async getMarginAccountInfo(): Promise<{ leverage?: number; [key: string]: unknown }> { return this.orderService.getMarginAccountInfo(); }
   calculateMarginLeverage(marginData: BinanceFuturesAccountInfo | Record<string, unknown> | null): number { return this.orderService.calculateMarginLeverage(marginData); }
  ...
  async placeOrder(request: ActionPlaceOrderParams): Promise<OKXApiResponse> { return this.orderService.placeOrder(request); }
  async placeOrders(requests: ActionPlaceOrderParams[]): Promise<OKXApiResponse> { return this.orderService.placeOrders(requests); }
