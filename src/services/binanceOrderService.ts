...
import { _shouldLogBinanceError } from './binanceErrorDedupe.ts';
import { LogService } from './logService.ts';
import { isPendingNewState } from '../lib/orderStates.ts';

export class BinanceOrderService {
  private lastMarginQueryError: string | null = null;
  ...
  async amendBatchOrders(params: Array<Record<string, unknown>>) {
   ...
   async amendSpotOrderByRecreate(
   ...
   findSpotOtoGroupBySnapshot(openOrders: any[], target: any): { mainLeg: any; pendingLeg: any } | null {
   ...
   findSpotOtocoGroupBySnapshot(openOrders: any[], target: any): { mainLeg: any; tpLeg: any; slLeg: any } | null {
    const targetTime = Number(target?.cTime ?? target?.time ?? 0);
    if (!Number.isFinite(targetTime) || targetTime <= 0) return null;
    const targetSide = String(target?.side || "").toUpperCase();
    const targetType = String(target?.type || "").toUpperCase();
   ...
   async amendSpotOtoRecreateByGroup(
    item: Record<string, unknown>,
    symbol: string,
    mainLeg: any,
    pendingLeg: any
   ...
   async amendSpotOtocoRecreateByGroup(
    item: Record<string, unknown>,
    symbol: string,
    mainLeg: any,
    tpLeg: any,
   ...
   async amendSpotOcoRecreateByPair(
    item: Record<string, unknown>,
    symbol: string,
    tpLeg: any,
    slLeg: any,
   ...
   async amendFuturesAlgoOrderByRecreate(
    item: Record<string, unknown>,
    instId: string,
    symbol: string,
    algoId: string
   ...
   async amendMarginOrderByRecreate(
    item: Record<string, unknown>,
    instId: string,
    symbol: string,
    ordId: string
   ...
   findMarginOcoPairBySnapshot(openOrders: any[], target: any): { tpLeg: any; slLeg: any } | null {
    const targetTime = Number(target?.cTime ?? target?.time ?? 0);
   ...
   async amendMarginOcoRecreateByPair(
    item: Record<string, unknown>,
    symbol: string,
    side: string,
    isIsolated: boolean,
  ...
  async getOpenOrders(instId?: string, reqInstType?: string): Promise<any[]> {
    ...
    if (instId) {
      ...
      if (isMargin) {
        try {
          ...
          const errMsg = e instanceof Error ? e.message : String(e);
          if (_shouldLogBinanceError(`getOpenOrders:margin:${errMsg}`)) {
   ...
   calculateMarginLeverage(marginData: any): number {
  ...
  async getMarginPositions(instId?: string): Promise<any[]> {
    ...
    this.lastMarginQueryError = null;
    ...
    try {
      ...
      const errMsg = e instanceof Error ? e.message : String(e);
      if (_shouldLogBinanceError(`marginLeverage:${errMsg}`)) {
        LogService.logKey("system", 'account.margin.info.failed', { label: this.sdk.accountLabel(), msg: errMsg, exchange: 'BINANCE' }, 'error');
      }
    }
    ...
    try {
      ...
      const errMsg = e instanceof Error ? e.message : String(e);
      if (_shouldLogBinanceError(`marginIsolated:${errMsg}`)) {
        LogService.logKey("system", 'account.margin.position.failed', { label: this.sdk.accountLabel(), msg: errMsg }, 'warn');
      }
    }
    ...
    try {
      ...
      const errMsg = e instanceof Error ? e.message : String(e);
      this.lastMarginQueryError = errMsg;
      if (_shouldLogBinanceError(`marginCross:${errMsg}`)) {
        LogService.logKey("system", 'account.margin.position.failed', { label: this.sdk.accountLabel(), msg: errMsg }, 'error');
      }
    }
  ...
  async closePosition(params: {
    ...
    if (!isFutures) {
      ...
      try {
        ...
        if (!target) {
          const queryErr = this.lastMarginQueryError;
          const detail = queryErr ? `（注意：仓位查询本身失败 — ${queryErr}）` : '';
          LogService.warn("close", `[BINANCE] No open margin position found for ${params.instId} in ${tdMode} mode.${detail}`);
          return this.sdk.fail(`No open margin position found for ${params.instId} in ${tdMode} mode.${detail}`);
        }
