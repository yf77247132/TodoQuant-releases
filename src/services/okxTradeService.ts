...
export class OKXTradeService {
  ...
  async getHistoryPositions(instType?: string, instId?: string, limit?: number, after?: string): Promise<OKXPositionHistory[]> {
      ...
      try {
        ...
        if (data.code === '0' && Array.isArray(data.data)) {
          if (data.data.length > 0) {
            ...
            if (data.data.length < pageSize) break;
            afterCursor = data.data[data.data.length - 1].uTime;
          } else {
            break;
          }
        } else {
  ...
  private mapToOKXPayload(request: ActionPlaceOrderParams): Record<string, unknown> {
    ...
    const isSpot = SymbolUtils.isSpotInstId(instId);
