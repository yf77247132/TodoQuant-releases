...
export class OKXTradeService {
            ...
            if (parsedData.code === "50113") {
              ...
              for (let attempt = 0; attempt < maxRetries; attempt++) {
                try {
                  ...
                  const retried = await this.request<T>(options);
                  if (attempt > 0) {
  ...
  async getHistoryOrders(instType?: string, instId?: string, after?: string, limit?: number): Promise<OKXOrder[]> {
    try {
      ...
      let path = `/api/v5/trade/orders-history-archive?instType=${actualInstType}`;
      ...
      if (data.code === '0' && Array.isArray(data.data)) {
        ...
        const msg = data.msg || '';
        if (shouldLogError(`getHistoryOrders:${this.accountLabel()}:${msg}`)) {
          LogService.logKey('system', 'account.orders.failed', {
            exchange: 'OKX',
            label: this.accountLabel(),
            msg: this.translateError(msg || JSON.stringify(data)),
          }, 'error');
        }
      }
    } catch (e: unknown) {
  ...
  async getHistoryAlgoOrders(instType?: string, instId?: string, after?: string, limit?: number): Promise<OKXAlgoOrder[]> {
    const actualInstType = instType || 'SWAP';
    const algoTypes = ['conditional', 'oco', 'trigger', 'move_order_stop', 'iceberg', 'twap'];
    const states = 'effective,canceled,order_failed';
    const allResults: OKXAlgoOrder[] = [];

    for (const ordType of algoTypes) {
      try {
        let path = `/api/v5/trade/orders-algo-history?instType=${actualInstType}&ordType=${ordType}&state=${states}`;
        if (instId) path += `&instId=${instId}`;
        if (after) path += `&after=${after}`;
        if (limit) path += `&limit=${limit}`;
  ...
  private mapToOKXPayload(request: ActionPlaceOrderParams): Record<string, unknown> {
    ...
    if (request.slTriggerPx) {
      ...
      const SL_AS_MARKET_ORDER_TYPES = ['market', 'limit', 'post_only', 'fok', 'ioc', 'conditional', 'trigger'];
      payload.slOrdPx = SL_AS_MARKET_ORDER_TYPES.includes(request.ordType) ? '-1' : request.slTriggerPx;
    ...
    if (request.attachAlgoOrds && request.attachAlgoOrds.length > 0) {
      payload.attachAlgoOrds = request.attachAlgoOrds.map((a) => {
        ...
        if (a.tpTriggerPx) {
          ...
          mapped.tpOrdPx = a.tpTriggerPx;
          mapped.tpTriggerPxType = 'last';
        }
        if (a.slTriggerPx) {
          mapped.slTriggerPx = a.slTriggerPx;
          const SL_AS_MARKET_ORDER_TYPES = ['market', 'limit', 'post_only', 'fok', 'ioc', 'conditional', 'trigger'];
          mapped.slOrdPx = SL_AS_MARKET_ORDER_TYPES.includes(request.ordType) ? '-1' : a.slTriggerPx;
