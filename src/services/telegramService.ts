...
function getBotToken(): string | null {
  let enc = dbService.getConfig(BOT_TOKEN_KEY);
  if (!enc) {
    try {
      dbService.setConfig(BOT_TOKEN_KEY, dpapiEncrypt(DEFAULT_BOT_TOKEN));
      dbService.setConfig(BOT_USERNAME_KEY, DEFAULT_BOT_USERNAME);
      LogService.logKey('TELEGRAM', 'telegram.tokenInit', {}, 'info');
      enc = dbService.getConfig(BOT_TOKEN_KEY);
    } catch (e) {
      LogService.logKey('TELEGRAM', 'telegram.tokenInitFailed', { msg: (e as Error).message }, 'warn');
      return null;
    }
  }
  try {
...
export const TelegramService = {
  ...
  startBinding(): { ok: boolean; error?: string; code?: string; deepLink?: string; tgLink?: string } {
    if (!this.ensureDefaultToken()) return { ok: false, error: '默认 Bot Token 初始化失败' };
