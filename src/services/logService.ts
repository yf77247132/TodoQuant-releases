...
export class LogService {
  ...
  private static readonly AUTH_ERROR_CACHE = new Map<string, number>();
  private static readonly AUTH_ERROR_TTL_MS = 60000;
  private static readonly AUTH_ERROR_MARKERS = ['Invalid API-key', 'PASSPHRASE incorrect', '50111'];
  ...
  static addLog(scriptType: string, message: string, level: 'info' | 'warn' | 'error' | 'debug' = 'info', category?: ErrorCategory, configId?: string) {
    this.addLogWithMeta(scriptType, message, level, category, configId);
  }
  ...
  static logKey(
    ...
    const authMsg = String(params.msg || '');
    if (key.endsWith('.failed') && this.AUTH_ERROR_MARKERS.some(m => authMsg.includes(m))) {
      const accountKey = String(params.label ?? params.accountIdx ?? 'unknown');
      if (this.shouldSuppressAuthError(accountKey)) return;
    }
  ...
  private static shouldSuppressAuthError(accountKey: string): boolean {
    const now = Date.now();
    const last = this.AUTH_ERROR_CACHE.get(accountKey);
    if (last && (now - last) < this.AUTH_ERROR_TTL_MS) {
      return true;
    }
    this.AUTH_ERROR_CACHE.set(accountKey, now);
    for (const [k, t] of this.AUTH_ERROR_CACHE.entries()) {
      if (now - t > this.AUTH_ERROR_TTL_MS * 2) this.AUTH_ERROR_CACHE.delete(k);
    }
    return false;
  }
  ...
  private static addLogWithMeta(
    ...
    if (wsManager && typeof wsManager.sendLog === 'function') {
      ...
      wssRef.clients.forEach((client) => {
        if (client.readyState === WebSocket.OPEN && client.bufferedAmount < 4 * 1024 * 1024) { client.send(payload); }
      });
    }
  }
}
