...
import { requireOkxWsManager, registerAccountMonitorAccess } from "../lib/wsServiceRegistry.ts";
import { SymbolUtils } from "../lib/symbolUtils.ts";
import { isAlgoOrderType, LIVE_STATES } from "../lib/orderStates.ts";
import type { OrderTypeLike } from "../lib/orderStates.ts";
...
export class AccountMonitor extends EventEmitter {
  ...
  async startMonitoring(accountIdx: number, intervalMs: number = 10000) {
    ...
    const sync = async () => {
      const instance = requireOkxWsManager().instance;
  ...
  public async triggerSyncByAccountId(accountId: string) {
    ...
    if (accountIdx !== undefined) {
      ...
      const runSyncPositions = async (tag: string) => {
        ...
        try {
          await this.syncPositions(idx);
        } catch (err: unknown) {
          LogService.warn("monitor", `账户 ${idx} 持仓同步失败 (${tag}): ${(err as Error).message}`);
        }
      };
      ...
      const prevTimers = this.triggerTimers[idx];
      if (prevTimers && prevTimers.length > 0) {
        prevTimers.forEach(t => clearTimeout(t));
      }
      this.triggerTimers[idx] = [
        setTimeout(() => runSyncPositions("快速持仓"), 500),
  ...
  getCachedService(accountId: string): OKXTradeService | BinanceSdkTradeService | null {
    const accountIdx = Object.keys(this.accounts).find(idx =>
      this.accounts[Number(idx)].accountId === accountId
    );
    if (accountIdx === undefined) return null;
    return this.services[Number(accountIdx)] || null;
  }
  ...
  private static isTriggeredTriggerAlgo(order: AccountOrder): boolean {
    return isAlgoOrderType(order) && String(order.state || '').toLowerCase() === 'effective';
  }
  ...
  private async fetchAllPendingOrders(service: OKXTradeService | BinanceSdkTradeService, algoTypes?: string[], instType?: string): Promise<AccountOrder[]> {
    if (algoTypes && algoTypes.length > 0) {
      ...
      return results.flat()
        .filter(o => !AccountMonitor.isTriggeredTriggerAlgo(o))
        .map(order => AccountMonitor.transformOrder(order) as AccountOrder);
    }
  ...
  async syncAccountOrders(accountIdx: number, algoTypes?: string[], instType?: string) {
    ...
    if (newOrders !== null) {
      ...
      const isQueried = (o: AccountOrder): boolean => {
        if (newOrders.length > 0) return actualTypes.has(o.ordType as string);
        if (algoTypes && algoTypes.length > 0) return (algoTypes as string[]).includes(o.ordType as string);
        if (algoTypes && algoTypes.length === 0) return !isAlgoOrderType(o as unknown as OrderTypeLike);
        return true;
      };
      const otherOrders = data.orders.filter(o => !isQueried(o));

      const REST_MISSING_RETAIN_MS = 30_000;
      const orderKey = (o: AccountOrder): string =>
        isAlgoOrderType(o as unknown as OrderTypeLike) ? (o.algoId || o.ordId) : o.ordId;
      ...
      const retainRestMissing = (candidates: AccountOrder[]): AccountOrder[] =>
        candidates.filter((o) => {
          if (!isQueried(o)) return false;
          const id = orderKey(o);
          if (!id) return false;
          if (newIds.has(id)) {
            ordersCache?.clearRestMissing(accountIdx, id);
            return false;
          }
          const state = String(o.state || '').toLowerCase();
          if (!LIVE_STATES.has(state)) return false;
          if (isAlgoOrderType(o as unknown as OrderTypeLike) && state === 'effective') return false;
          const missAt = ordersCache?.getRestMissingAt(accountIdx, id) || 0;
          if (!missAt) {
            ordersCache?.markRestMissing(accountIdx, id, nowMs);
            return true;
          }
          return nowMs - missAt <= REST_MISSING_RETAIN_MS;
        });

      const retainedData = retainRestMissing(data.orders);
      if (retainedData.length > 0) {
        LogService.addLog('order-ghost', `定向同步: 账户#${accountIdx} REST返回=${newOrders.length} 防吞单保留=${retainedData.length}单(限30s) id=${retainedData.slice(0, 5).map(o => orderKey(o)).filter(Boolean).join(',')}`, 'debug');
      }
      const seen = new Set<string>();
      const deduped = [...otherOrders, ...retainedData, ...newOrders].filter((o) => {
        const id = orderKey(o);
        if (!id) return true;
      ...
      if (ordersCache) {
        const cacheOrders = ordersCache.get(accountIdx).data;
        const cacheOtherOrders = cacheOrders.filter((o) => !isQueried(o));
        const retainedCache = retainRestMissing(cacheOrders).filter((o) => {
          const id = orderKey(o);
          return !(id && ordersCache.isRecentlyRemoved(accountIdx, id));
        });
        const cacheSeen = new Set<string>();
        const cacheDeduped = [...cacheOtherOrders, ...retainedCache, ...newOrders].filter((o) => {
          const id = orderKey(o);
          if (!id) return true;
  ...
  public static transformOrder(order: Record<string, unknown>): Record<string, unknown> {
    ...
    if (isAlgo) {
      ...
      addIfExist('tpPrice', order.tpTriggerPx || order.attachAlgoOrds?.[0]?.tpTriggerPx);
      addIfExist('slPrice', order.slTriggerPx || order.attachAlgoOrds?.[0]?.slTriggerPx);
      addIfExist('tpOrdPx', order.tpOrdPx || order.attachAlgoOrds?.[0]?.tpOrdPx);
      addIfExist('slOrdPx', order.slOrdPx || order.attachAlgoOrds?.[0]?.slOrdPx);
  ...
  public static mergeBinanceOtoOrders<T extends AccountOrder>(orders: T[]): T[] {
    ...
    const processOtoGroup = (group: T[]): void => {
    ...
    orderListGroups.forEach((group) => processOtoGroup(group));

    timeGroups.forEach((group) => processOtoGroup(group));
  ...
  getAllPositions(): OKXPosition[] {
    return Object.values(this.accounts).flatMap((d) => d?.positions || []);
  }
  ...
  updatePositionsFromWs(accountIdx: number, positions: OKXPosition[]) {
    ...
    const mergeKey = (p: OKXPosition): string =>
      (p as { posId?: string }).posId
        ? String((p as { posId?: string }).posId)
        : `${p.instId}_${p.posSide}`;
    const isEmpty = (p: OKXPosition): boolean =>
      (!p.pos || p.pos === '0') && (!(p as { availPos?: string }).availPos || (p as { availPos?: string }).availPos === '0');

    const map = new Map<string, OKXPosition>();
    data.positions.forEach(p => map.set(mergeKey(p), p));
    positions.forEach(p => {
      const key = mergeKey(p);
      if (isEmpty(p)) {
        map.delete(key);
      } else {
        map.set(key, { ...map.get(key), ...p } as OKXPosition);
      }
    });
    ...
    this.emit('positions_updated', { accountIdx, positions: merged });
  }
  ...
  updateOrdersFromWs(accountIdx: number, orders: AccountOrder[]) {
    ...
    const orderKey = (o: AccountOrder): string => {
      return isAlgoOrderType(o) ? (o.algoId || o.ordId) : o.ordId;
    };

    for (const newOrder of orders) {
      const ordId = orderKey(newOrder);
      if (!ordId) continue;
      ...
      if (isAlgoEffective) {
        data.orders = data.orders.filter(o => orderKey(o) !== ordId);
      } else if (isLive) {
        const existingIdx = data.orders.findIndex(o => orderKey(o) === ordId);
        if (existingIdx >= 0) {
...
registerAccountMonitorAccess(
  () => AccountMonitor.getInstance(),
  AccountMonitor.transformOrder.bind(AccountMonitor)
);
