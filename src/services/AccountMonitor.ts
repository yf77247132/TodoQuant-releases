    ...
    if (newOrders !== null) {
      ...
      const retainRestMissing = (candidates: AccountOrder[]): AccountOrder[] =>
        candidates.filter((o) => {
          ...
          const id = orderKey(o);
          if (!id || newIds.has(id)) return false;
          ...
          const missAt = Number(o._restMissingAt || 0);
          if (!missAt) {
            o._restMissingAt = nowMs;
            return true;
          }
      ...
      if (retainedData.length > 0) {
        LogService.info('order-ghost', `[ORDER-GHOST] 定向同步: 账户#${accountIdx} REST返回=${newOrders.length} 防吞单保留=${retainedData.length}单(限30s)`);
      }
