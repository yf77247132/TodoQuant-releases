...
import { requireAccountMonitor, registerOkxWsManagerAccess } from '../lib/wsServiceRegistry.ts';
import { planWsFlush } from '../lib/wsBatchBuffer.ts';
...
export class OKXWebSocketManager {
  ...
  private pongWaitTimers: Record<string, NodeJS.Timeout> = {};
  ...
  private flushMessageBuffer() {
    ...
    const { frames, dropped } = planWsFlush(this.messageBuffer);
    if (dropped > 0) {
    ...
    const groupTasks = Array.from(exchangeGroups.entries()).map(async ([exchange, groupAccounts]) => {
      for (const { idx, acc } of groupAccounts) {
        ...
        if (!isDecryptionFailed && hasBasicCerts && hasOkxCerts) {
          try {
            await requireAccountMonitor().startMonitoring(idx);
          } catch (err) {
  ...
  private startPing(ws: WebSocket, accountIdx: number, wsType: 'private' | 'business'): void {
    startPingCore(this.pingTimers, this.businessPingTimers, this.pongWaitTimers, this.PING_INTERVAL_MS, ws, accountIdx, wsType);
  }

  private stopPing(accountIdx: number, wsType: 'private' | 'business'): void {
    stopPingCore(this.pingTimers, this.businessPingTimers, this.pongWaitTimers, accountIdx, wsType);
  }
  ...
  stopAll(): void {
    ...
    Object.values(this.pongWaitTimers).forEach(t => clearTimeout(t));
    this.pongWaitTimers = {};
  }
  ...
  permanentlyStop(accountIdx: number): void {
    ...
    const monitor = requireAccountMonitor();
  ...
  private createOkxWsInternal(
    ...
    createOkxWsInternalCore({
      ...
      activeClients,
      clearLoginTimeout,
      pongWaitTimers: this.pongWaitTimers,
      ...
      handleDataMessage: this.handleDataMessage.bind(this),
...
registerOkxWsManagerAccess(() => OKXWebSocketManager);
