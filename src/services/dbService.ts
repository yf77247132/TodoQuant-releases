...
function isRealDbCorruption(dbError: unknown): boolean {
  const err = dbError as { code?: string; message?: string } | null;
  const code = err?.code ?? '';
...
const CORRUPTED_KEEP = 10;

function pruneCorruptedBackups() {
  try {
    const dir = getCorruptedDirPath();
    if (!fs.existsSync(dir)) return;
    const files = fs.readdirSync(dir)
      .filter((f) => f.startsWith('trading.db.') && f.endsWith('.corrupted'))
      .sort();
    const excess = files.length - CORRUPTED_KEEP;
