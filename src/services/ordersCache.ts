...
import { runBatched } from "../lib/runBatched.ts";
import { sweepRestMissingGrace } from "../lib/restMissingGrace.ts";
...
export class OrdersCacheManager {
  ...
  private restMissingAt: Record<number, Record<string, number>> = {};
  private accountIdxToIdMap: Record<number, string> = {};
  private skipMergeOnce = false;

  constructor(_config: OrdersCacheConfig) {}

  private static orderKey(order: AccountOrder): string {
    return isAlgoOrderType(order) ? (order.algoId || order.ordId) : order.ordId;
  }
  ...
  private markRemovedOrder(accountIdx: number, orderId: string): void {
    ...
    this.clearRestMissing(accountIdx, orderId);
  }

  public getRestMissingAt(accountIdx: number, orderId: string): number {
    if (!orderId) return 0;
    return this.restMissingAt[accountIdx]?.[orderId] || 0;
  }

  public markRestMissing(accountIdx: number, orderId: string, at: number = Date.now()): void {
    if (!orderId) return;
    if (!this.restMissingAt[accountIdx]) this.restMissingAt[accountIdx] = {};
    if (!this.restMissingAt[accountIdx][orderId]) {
      this.restMissingAt[accountIdx][orderId] = at;
    }
  }

  public clearRestMissing(accountIdx: number, orderId: string): void {
    const bucket = this.restMissingAt[accountIdx];
    if (bucket && orderId) delete bucket[orderId];
  }

  public isRecentlyRemoved(accountIdx: number, orderId: string): boolean {
  ...
  updateOrder(accountIdx: number, order: AccountOrder): void {
    ...
    const ordId = isAlgoOrderType(order) ? (order.algoId || order.ordId) : order.ordId;
    if (!ordId) return;
    ...
    const index = orders.findIndex(o => {
      const oKey = isAlgoOrderType(o) ? (o.algoId || o.ordId) : o.ordId;
      return oKey === ordId;
    });
  ...
  removeOrder(accountIdx: number, ordId: string): void {
    ...
    this.cache[accountIdx].data = orders.filter(o => {
      if (isAlgoOrderType(o)) {
        return (o.algoId || o.ordId) !== ordId;
      }
      return o.ordId !== ordId;
    });
  ...
  async preload(): Promise<void> {
    ...
    const REFRESH_BASE_MS = 10_000;
    const REFRESH_JITTER_MS = 2_000;
    const scheduleNext = (): void => {
      const delay = REFRESH_BASE_MS + Math.floor(Math.random() * REFRESH_JITTER_MS);
      this.refreshInterval = setTimeout(() => {
        refresh().catch((e: unknown) => {
          const msg = e instanceof Error ? e.message : String(e);
          LogService.warn("system", `[OrdersCache] 定时刷新失败（等待下一轮重试）: ${msg}`);
        });
        scheduleNext();
      ...
      if (hasCredentials) {
        ...
        const accountTask = (async () => {
          try {
            ...
            ErrorMonitor.captureError(error, ErrorLevel.MEDIUM, ErrorCategory.SYSTEM, { accountIdx: idx });
            if (this.cache[idx]) {
    ...
    Object.keys(newCache).forEach(idx => {
      ...
      const liveApiOrders = options.filterRecentlyRemoved
        ? apiOrders.filter(o => {
            ...
            const id = OrdersCacheManager.orderKey(o);
      ...
      const apiIds = new Set(liveApiOrders.map(o => OrdersCacheManager.orderKey(o)));

      const REST_MISSING_GRACE_MS = 30_000;
      const graceCandidateIds: string[] = [];
      existingData.forEach(o => {
        const id = OrdersCacheManager.orderKey(o);
        if (!id || apiIds.has(id)) return;
        if (!LIVE_STATES.has(String(o.state || '').toLowerCase())) return;
        if (this.isRecentlyRemoved(numIdx, id)) return;
        graceCandidateIds.push(id);
      });
      const { keptIds: ghostKept, expireIds: ghostTombstoned, nextMarkers } = sweepRestMissingGrace({
        prevMarkers: this.restMissingAt[numIdx] || {},
        candidateIds: graceCandidateIds,
        now,
        graceMs: REST_MISSING_GRACE_MS,
      });
      this.restMissingAt[numIdx] = nextMarkers;
      ghostTombstoned.forEach(id => this.markRemovedOrder(numIdx, id));
      if (ghostTombstoned.length > 0) {
        LogService.info('order-ghost', `轮询合并: 账户#${numIdx} REST返回=${liveApiOrders.length} 宽限保留=${ghostKept.length}单 墓碑剔除=${ghostTombstoned.length}单 id=${ghostTombstoned.slice(0, 5).join(',')}`);
      } else if (ghostKept.length > 0) {
        LogService.addLog('order-ghost', `轮询合并: 账户#${numIdx} REST返回=${liveApiOrders.length} 宽限保留=${ghostKept.length}单 墓碑剔除=0单 id=${ghostKept.slice(0, 5).join(',')}`, 'debug');
      }
      ...
      const extraFromWs = existingData.filter(o => {
        ...
        if (isAlgoOrderType(o) && String(o.state || '').toLowerCase() === 'effective') {
          return false;
        }
        return true;
      });
    ...
    if (options.applyRaceProtection) {
      Object.keys(newCache).forEach(idx => {
        ...
        if (existing && existing.timestamp > now) {
          const existingOrders = existing.data || [];
          const freshOrders = newCache[numIdx]?.data || [];
          const existingIds = new Set(existingOrders.map(o => OrdersCacheManager.orderKey(o)));
          newCache[numIdx] = {
            data: [
              ...existingOrders,
              ...freshOrders.filter(o => {
                ...
                if (this.isRecentlyRemoved(numIdx, id)) return false;
                if (!LIVE_STATES.has(String(o.state || '').toLowerCase())) return false;
                return true;
              }),
            ],
  ...
  private async getPendingOrders(accountIdx: number, acc: ExchangeAccount): Promise<AccountOrder[]> {
    ...
    try {
      if (isBinance) {
        ...
        const regularOrderBatches = await runBatched(
          instTypes.map(type => () => service.fetchAllPagesForOrders(type === 'SWAP' ? undefined : type)),
          5,
          200
        );
        ...
        const algoOrderBatches = await runBatched(
          algoTypes.map(type => () => service.fetchAllPagesForAlgoType(type)),
          5,
          200
        );
        ...
        algoOrderBatches.forEach((orders) => {
          if (orders && orders.length > 0) {
            orders.forEach((o) => {
              const algoState = String(o.state || '').toLowerCase();
              if (isAlgoOrderType(o) && algoState === 'effective') return;
