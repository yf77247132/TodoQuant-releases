...
export class OrdersCacheManager {
  ...
  private async refreshSingleRound(options: {
    ...
    Object.keys(newCache).forEach(idx => {
      ...
      const ghostKept: string[] = [];
      const ghostTombstoned: string[] = [];
      existingData.forEach(o => {
        ...
        if (!missAt) {
          o._restMissingAt = now;
          ghostKept.push(id);
        } else if (now - missAt > REST_MISSING_GRACE_MS) {
          this.markRemovedOrder(numIdx, id);
          ghostTombstoned.push(id);
        }
      });
      if (ghostKept.length > 0 || ghostTombstoned.length > 0) {
        LogService.info('order-ghost', `[ORDER-GHOST] 轮询合并: 账户#${numIdx} REST返回=${liveApiOrders.length} 宽限保留=${ghostKept.length}单 墓碑剔除=${ghostTombstoned.length}单${ghostTombstoned.length > 0 ? ` id=${ghostTombstoned.slice(0, 5).join(',')}` : ''}`);
      }
