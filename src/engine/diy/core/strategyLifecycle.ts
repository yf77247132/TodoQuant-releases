...
export interface StrategyLifecycleDeps {
  ...
  executingRefCount: Map<string, number>;
...
export async function initCore(deps: StrategyLifecycleDeps): Promise<void> {
  ...
  const strategies = safeJsonParse<DIYStrategy[]>(strategiesRaw, [], {
    context: 'DiyEngine.init.diy_strategies',
    reporter: (error, meta) => deps.reportJsonParseError(meta.context, error, meta.rawPreview)
  });
  let resets = 0;
  for (const st of strategies) {
    const lastDay = st.lastTriggered ? new Date(st.lastTriggered).setHours(0, 0, 0, 0) : 0;
    if (lastDay > 0 && lastDay < today) {
      st.triggerCount = 0;
      st.lastTriggered = 0;
      resets++;
    }
  ...
  if (resets > 0) {
    dbService.setConfig('diy_strategies', JSON.stringify(strategies));
    LogService.logKey('diy', 'diy.dailyReset.onBoot', { count: resets }, 'info');
  }
...
export async function startStrategyCore(deps: StrategyLifecycleDeps, strategyId: string): Promise<void> {
  ...
  if (existing) {
    ...
    deps.runningStrategies.set(String(st.id), { ...st, triggerCount: st.triggerCount ?? 0, lastTriggered: st.lastTriggered ?? 0 });
  }
...
export async function stopStrategyCore(deps: StrategyLifecycleDeps, strategyId: string): Promise<void> {
  ...
  deps.executingRefCount.delete(strategyId);
  ...
  for (const key of deps.crossState.keys()) {
    if (key.startsWith(`${strategyId}:`)) {
      deps.crossState.delete(key);
    }
  }
  for (const key of deps.instrumentTriggered.keys()) {
    if (key.startsWith(`${strategyId}:`)) {
      deps.instrumentTriggered.delete(key);
    }
  }
  deps.conditionState.delete(`__signalWarned:${strategyId}`);
}
