...
export async function executeActionsCore(
  ...
  runToken: number,
  triggeredInstId?: string
): Promise<void> {
  if (!deps.isStrategyRunActive(strategy.id, runToken)) return;
  deps.executingStrategies.add(strategy.id);
  deps.executingRefCount.set(strategy.id, (deps.executingRefCount.get(strategy.id) || 0) + 1);
  ...
  try {
    await LogService.runWithRedirect('diy', strategy.id, async () => {
      ...
      for (let i = 0; i < strategy.actions.length; i++) {
        ...
        try {
          await dispatchActionCore(deps, action, triggerAccountIdx, appConfig, strategy.testMode, strategy.id, strategy, triggeredInstId);
        } catch (e: unknown) {
    ...
    const refCount = (deps.executingRefCount.get(strategy.id) || 1) - 1;
    if (refCount <= 0) {
      deps.executingStrategies.delete(strategy.id);
      deps.executingRefCount.delete(strategy.id);
    } else {
      deps.executingRefCount.set(strategy.id, refCount);
    }
  }
}
...
export async function dispatchActionCore(
  ...
  switch (action.type) {
    case 'place_order': {
      ...
      await PlaceOrderAction.execute(env.config, env.tradeService, undefined, env.config.id, appConfig, triggeredInstId);
      break;
    }
    case 'amend_order': {
    ...
    case 'close_pos': {
      ...
      await ClosePositionAction.execute(env.config, env.tradeService, undefined, triggeredInstId);
      break;
    }
