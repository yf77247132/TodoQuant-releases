...
import { resolveAccountNameById } from '../../../lib/resolveAccount.ts';
...
const ACCOUNT_FREE_CONDITION_TYPES = [
  'time_window',
  'cooldown',
  'count_limit',
  'tv_signal',
...
function isAccountFreeCondition(type: string): boolean {
  return (ACCOUNT_FREE_CONDITION_TYPES as readonly string[]).includes(type);
}

const POSITIONS_SENTINEL = '@positions';
...
function resolveAccountName(params: Record<string, unknown>, accountIdx: number, appConfig: AppConfig): string {
  ...
  const found = resolveAccountNameById(aid, appConfig?.accounts, appConfig?.accountNames);
...
export function evaluateTemplateCore(
  ...
  for (let i = 0; i < template.conditions.length; i++) {
    ...
    const accountId = isAccountFreeCondition(condition.type)
      ? ''
      : String(condition.params?.accountId || '').trim();
    let targetIdx = triggerAccountIdx;
    if (accountId) {
      const foundIdx = appConfig.accounts
        ? appConfig.accounts.findIndex((a) => a.id === accountId)
        : -1;
      if (foundIdx < 0) {
        if (Date.now() % 60000 < 1100) {
          LogService.logKey('diy', 'diy.cond.accountMissing', { i: i + 1, accountId }, 'warn', strategy.id);
        }
        results.push({
          idx: i,
          type: condition.type,
          matched: false,
          triggeredInstIds: [],
          pendingCrossUpdates: new Map<string, boolean>(),
          pendingTriggeredUpdates: new Map<string, number>(),
        });
        allMatched = false;
        continue;
      }
  ...
  if (hadPendingSignal && !allMatched) {
    ...
    const allFailedAreIndicator = hasFtSignal && !intersectionDowngraded && failed.every(r => r.type === 'indicator');
...
export function evaluateSingleConditionCore(
  ...
  if (!accountData && !isAccountFreeCondition(type)) {
    if (now % 30000 < 1000) LogService.logKey('diy', 'diy.condition.notReady', { id: configId }, 'warn', configId);
    return { matched: false, triggeredInstIds: [], pendingCrossUpdates: new Map<string, boolean>(), pendingTriggeredUpdates: new Map<string, number>() };
  }
...
export function doEvaluateCore(
  ...
  switch (type) {
    ...
    case 'price_change_low24h': {
      const exchange = sp.exchange || 'okx';
      const instId = sp.instId || '';
      const operator = sp.operator || '>';
      const target = parseFloat(sp.threshold || '0');
      const metric = type === 'price_change_today' ? 'today' : type === 'price_change_high24h' ? 'high24h' : type === 'price_change_low24h' ? 'low24h' : '24h';
      const changeVal = (ticker: TickerData): number => {
        if (metric === 'high24h' || metric === 'low24h') {
          const base = ticker.sodUtc8;
          if (!Number.isFinite(base) || base <= 0) return NaN;
          return (ticker[metric as 'high24h' | 'low24h'] / base - 1) * 100;
        }
      ...
      if (instId && instId !== POSITIONS_SENTINEL) {
      ...
      let scopeTickers: TickerData[];
      let scopeLabel: string;
      if (instId === POSITIONS_SENTINEL) {
        const allPos = AccountMonitor.getInstance().getAllPositions();
        const ids = [...new Set(allPos.map((p) => p?.instId || '').filter(Boolean))];
        if (ids.length === 0) return r(false, '无持仓');
        scopeTickers = ids
          .map((iid) => marketScanner.getTicker(exchange, iid))
          .filter((tk): tk is TickerData => Boolean(tk));
        scopeLabel = '当前持仓';
      } else {
        scopeTickers = marketScanner.getAllTickers(exchange);
        scopeLabel = '';
      }

      const idleTakeMin = operator === 'cross_below' || operator === '<' || operator === '<=';
      const matched: string[] = [];
      const skipped: string[] = [];
      let idleVal = idleTakeMin ? Infinity : -Infinity;
      for (const ticker of scopeTickers) {
        const val = changeVal(ticker);
        if (isNaN(val)) continue;
        if (idleTakeMin ? val < idleVal : val > idleVal) idleVal = val;
      ...
      const idleNum = Number.isFinite(idleVal) ? idleVal : 0;
      const idleLabel = `${scopeLabel}${idleTakeMin ? '最低' : '最高'}${idleNum.toFixed(2)}%`;
      return rWithPending(
        matched.length > 0,
        matched.length > 0 ? `${matched.length}个交易对满足(${names}${suffix})${skipNames}` : idleLabel,
        matched,
        pendingCross,
        pendingTriggered
      );
...
export function getEvaluationLogMessageCore(
  ...
  if (!accountData && !isAccountFreeCondition(type)) return null;
  ...
  switch (type) {
    ...
    case 'price_change_low24h': {
      ...
      const instId = sp.instId || '';
      const opL = OPERATOR_LOG_LABELS[sp.operator || '>'] || sp.operator;
      const metricLabel = type === 'price_change_today' ? '当日' : type === 'price_change_high24h' ? '24h最高价' : type === 'price_change_low24h' ? '24h最低价' : '24h';
      const metricSuffix = type === 'price_change_low24h' ? '跌幅' : '涨幅';
      const showVal = (ticker: TickerData | undefined): string => {
        ...
        const v = type === 'price_change_today' ? ticker.changeToday
          ...
          : type === 'price_change_low24h' ? (Number.isFinite(ticker.sodUtc8) && ticker.sodUtc8 > 0 ? (ticker.low24h / ticker.sodUtc8 - 1) * 100 : NaN)
          : ticker.change24h;
      ...
      if (instId && instId !== POSITIONS_SENTINEL) {
        ...
        return `${t('engine.detecting')}: ${typeLabel}，${exchange}/${instId}，${metricLabel}${metricSuffix}: ${val}%，阈值: ${sp.threshold}%，判断逻辑: ${opL}，结果: ${resStr}`;
      }
      ...
      const scopeDisplay = instId === POSITIONS_SENTINEL ? t('condition.currentPositions') : '全部';
      return `${t('engine.detecting')}: ${typeLabel}，${exchange}/${scopeDisplay}，${metricLabel}${metricSuffix}: ${valDisplay}，阈值: ${sp.threshold}%，判断逻辑: ${opL}，结果: ${resStr}`;
    }
    case 'indicator': {
