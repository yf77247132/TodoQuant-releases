...
export interface ConditionEvaluatorDeps {
  ...
  instrumentTriggered: Map<string, boolean>;
...
export function evaluateTemplateCore(
  ...
  const results: { idx: number; type: string; matched: boolean; triggeredInstIds: string[]; pendingCrossUpdates: Map<string, boolean>; pendingTriggeredUpdates: Map<string, boolean> }[] = [];
  let allMatched = true;
  ...
  const triggeredInstIds = [...new Set(results.flatMap(r => r.triggeredInstIds))];
...
interface SingleConditionResult {
  ...
  pendingTriggeredUpdates: Map<string, boolean>;
}
...
export function doEvaluateCore(
  ...
  const EMPTY_TRIGGERED = new Map<string, boolean>();
  ...
  const rWithPending = (result: boolean, currentVal: string, triggeredInstIds: string[], pendingCrossUpdates: Map<string, boolean>, pendingTriggeredUpdates: Map<string, boolean>): DoEvaluateResult =>
  ...
  switch (type) {
    ...
    case 'price_change_today': {
      ...
      const pendingTriggered = new Map<string, boolean>();

      const isAlreadyTriggered = (iid: string): boolean => {
        if (allowRepeat) return false;
        return deps.instrumentTriggered.has(`${strategy.id}:${condIdx}:${iid}`);
      };
      ...
      if (instId) {
        ...
        if (isCrossAbove || isCrossBelow) {
          ...
          if (triggered) pendingTriggered.set(`${strategy.id}:${condIdx}:${instId}`, true);
        ...
        if (matched) pendingTriggered.set(`${strategy.id}:${condIdx}:${instId}`, true);
        ...
        if (isCrossAbove || isCrossBelow) {
          ...
          if (triggered) {
            if (isAlreadyTriggered(ticker.instId)) {
              ...
              pendingTriggered.set(`${strategy.id}:${condIdx}:${ticker.instId}`, true);
