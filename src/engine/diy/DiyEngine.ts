...
export class DiyEngine {
  ...
  private instrumentTriggered: Map<string, boolean> = new Map();

  private constructor() {}

  private resetDailyTriggeredKeys(): void {
    const dailyCondsByStrategy = new Map<string, Set<number>>();
    for (const [id, strategy] of this.runningStrategies.entries()) {
      ...
      const dailyConds = new Set<number>();
      template.conditions.forEach((cond, idx) => {
        const sp = cond.params as Record<string, string>;
        if (sp.repeat === 'daily') dailyConds.add(idx);
      });
      if (dailyConds.size > 0) dailyCondsByStrategy.set(id, dailyConds);
    }
    for (const key of this.instrumentTriggered.keys()) {
      const parts = key.split(':');
      const strategyId = parts[0];
      const condIdx = parseInt(parts[1], 10);
      const dailyConds = dailyCondsByStrategy.get(strategyId);
      if (dailyConds && dailyConds.has(condIdx)) {
    ...
    try {
      ...
      for (const st of arr) {
        ...
        for (const entry of triggered) {
          const key = `${String(st.id)}:${entry}`;
          this.instrumentTriggered.set(key, true);
        }
      }
  ...
  private flushPersistedStrategies() {
    ...
    try {
      ...
      for (const [key] of this.instrumentTriggered.entries()) {
        const firstColon = key.indexOf(':');
        if (firstColon === -1) continue;
        const strategyId = key.slice(0, firstColon);
        const entry = key.slice(firstColon + 1);
