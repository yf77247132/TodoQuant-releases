  ...
  private tick() {
    ...
    for (const [id, strategy] of this.runningStrategies.entries()) {
      try {
        ...
        const NON_ACCOUNT_CONDITIONS = new Set(['tv_signal', 'time_window', 'cooldown', 'count_limit', 'indicator', 'price_change', 'price_change_24h', 'price_change_today', 'price_change_high24h']);
        ...
        for (const accountIdx of accountIndices) {
          ...
          if (evalResult.triggeredInstIds.length > 0) {
            for (const instId of evalResult.triggeredInstIds) {
              void this.executeActions(strategy, accountIdx, runToken, instId);
            }
          } else {
