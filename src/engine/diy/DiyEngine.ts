...
import { evaluateTemplateCore, type TemplateEvalResult } from "./core/conditionEvaluator.ts";
import {
  executeActionsCore,
...
import { registerDiyEngineAccess } from "../../lib/wsServiceRegistry.ts";
...
export class DiyEngine {
  ...
  private executingRefCount: Map<string, number> = new Map();
  ...
  private crossState: Map<string, boolean> = new Map();

  private instrumentTriggered: Map<string, number> = new Map();

  private constructor() {}

  private getLocalMidnight(ts: number = Date.now()): number {
    return new Date(ts).setHours(0, 0, 0, 0);
  }

  private resetDailyTriggeredKeys(): void {
    const todayMidnight = this.getLocalMidnight();
    const dailyCondKeys = new Set<string>();
    for (const [id, strategy] of this.runningStrategies.entries()) {
      ...
      template.conditions.forEach((cond, idx) => {
        const sp = cond.params as Record<string, string>;
        if (sp.repeat === 'daily') {
          dailyCondKeys.add(`${id}:${idx}`);
        }
      });
    }
    for (const [key, triggeredMs] of this.instrumentTriggered.entries()) {
      const parts = key.split(':');
      const strategyId = parts[0];
      const condIdx = parts[1];
      if (!dailyCondKeys.has(`${strategyId}:${condIdx}`)) continue;
      if (triggeredMs !== todayMidnight) {
        this.instrumentTriggered.delete(key);
      }
    }
  }

  private restoreInstrumentTriggered(): void {
    try {
      const raw = dbService.getConfig("diy_strategies");
      const arr = safeJsonParse<DIYStrategy[]>(raw, [], {
        context: "DiyEngine.restoreInstrumentTriggered",
        reporter: (error, meta) => this.reportJsonParseError(meta.context, error, meta.rawPreview),
      });
      for (const st of arr) {
        const triggered = st.instrumentTriggered || [];
        for (const entry of triggered) {
          const parts = entry.split(':');
          let dateMs: number;
          let keySuffix: string;
          if (parts.length >= 3) {
            dateMs = parseInt(parts[parts.length - 1], 10);
            keySuffix = parts.slice(0, -1).join(':');
          } else {
            dateMs = 0;
            keySuffix = entry;
          }
          const key = `${String(st.id)}:${keySuffix}`;
          this.instrumentTriggered.set(key, dateMs);
        }
      }
      if (this.instrumentTriggered.size > 0) {
        LogService.logKey("diy", 'diy.restoreTriggered', { count: this.instrumentTriggered.size }, 'info');
      }
    } catch (e) {
      const err = e as Error;
      LogService.logKey("diy", 'diy.restoreTriggered.failed', { msg: err.message }, 'error');
    }
  }
  ...
  private buildLifecycleDeps(): StrategyLifecycleDeps {
    return {
      ...
      executingRefCount: this.executingRefCount,
      ...
      crossState: this.crossState,
      instrumentTriggered: this.instrumentTriggered,
    };
  }
  ...
  public async init() {
    ...
    this.restoreInstrumentTriggered();
    this.resetDailyTriggeredKeys();
  }
  ...
  private async syncPrefetchFundingRates(strategyId: string) {
    ...
    let appConfig: AppConfig = { accounts: [], accountNames: {} };
    try { appConfig = loadSavedConfig(); } catch {  }
    const instIds = this.collectFundingRateInstIdsForTemplate(template, appConfig);
  ...
  private tick() {
    ...
    const fundingInstIds = this.collectFundingRateInstIds(appConfig);
    ...
    this.prefetchFundingRatesFromInstIds(fundingInstIds);
    ...
    for (const [id, strategy] of this.runningStrategies.entries()) {
      try {
        ...
        const NON_ACCOUNT_CONDITIONS = new Set(['tv_signal', 'time_window', 'cooldown', 'count_limit', 'indicator', 'price_change', 'price_change_24h', 'price_change_today', 'price_change_high24h', 'price_change_low24h']);
        ...
        for (const accountIdx of accountIndices) {
          const evalResult = this.evaluateTemplate(strategy, template, accountIdx, appConfig);
          if (!evalResult.allMatched) continue;
          ...
          globalThis.OKX_WS_MANAGER?.broadcast?.({ type: 'strategy_toast', strategyName: strategy.name });

          const runActions = (instId?: string) => {
            void this.executeActions(strategy, accountIdx, runToken, instId).catch((e: unknown) => {
              ErrorMonitor.captureStrategyError(e as Error, { scriptType: 'diy', configId: id }, 'DiyEngine');
            });
          };
          if (evalResult.triggeredInstIds.length > 0) {
            for (const instId of evalResult.triggeredInstIds) {
              runActions(instId);
            }
          } else {
            runActions();
  ...
  private collectFundingRateInstIds(appConfig: AppConfig): Set<string> {
    const instIds = new Set<string>();
    for (const strategy of this.runningStrategies.values()) {
      const template = this.conditionTemplates.get(String(strategy.conditionTemplateId));
      if (!template) continue;
      this.collectFundingRateInstIdsForTemplate(template, appConfig, instIds);
    }
    return instIds;
  }

  private collectFundingRateInstIdsForTemplate(template: ConditionTemplate, appConfig: AppConfig, out?: Set<string>): Set<string> {
    const instIds = out || new Set<string>();
    for (const cond of template.conditions) {
      if (cond.type !== 'pos_funding_rate') continue;
      const params = (cond.params || {}) as Record<string, unknown>;
      const instId = String(params.instId || '').trim();
      if (instId) {
        instIds.add(instId);
        continue;
      }
      ...
      if (accountId && appConfig.accounts) {
        const idx = appConfig.accounts.findIndex((a) => a.id === accountId);
        if (idx >= 0) accountIdx = idx;
      }
      const data = AccountMonitor.getInstance().getAccountData(accountIdx);
      if (data?.positions) {
        for (const p of data.positions) instIds.add(p.instId);
      }
    }
    return instIds;
  }

  private prefetchFundingRatesFromInstIds(instIds: Set<string>) {
    for (const instId of instIds) {
  ...
  private flushPersistedStrategies() {
    ...
    try {
      ...
      for (const [key, dateMs] of this.instrumentTriggered.entries()) {
        const firstColon = key.indexOf(':');
        if (firstColon === -1) continue;
        const strategyId = key.slice(0, firstColon);
        const keySuffix = key.slice(firstColon + 1);
        const st = updatedById.get(strategyId);
        if (!st) continue;
        const arr = st.instrumentTriggered || [];
        const existingIdx = arr.findIndex(e => e.startsWith(`${keySuffix}:`) || e === keySuffix);
        const newEntry = `${keySuffix}:${dateMs}`;
        if (existingIdx >= 0) {
          arr[existingIdx] = newEntry;
        } else {
          arr.push(newEntry);
        }
        st.instrumentTriggered = arr;
      }
      for (const [, st] of updatedById.entries()) {
        const hasEntries = Array.from(this.instrumentTriggered.keys()).some(k => k.startsWith(`${String(st.id)}:`));
        if (!hasEntries && st.instrumentTriggered?.length) {
          st.instrumentTriggered = [];
        }
      }
  ...
  private async executeActions(strategy: DIYStrategy, triggerAccountIdx: number, runToken: number, triggeredInstId?: string) {
...
registerDiyEngineAccess(() => DiyEngine.getInstance());
