...
import type { ScriptState, ModuleConfig, StrategyError } from "../types/strategy.ts";
import type { ScriptType } from "../types/strategy.ts";
import { Exchange } from "../types/core.ts";
import {
  buildAmendDescSegments,
  buildAmendDescText,
  buildCancelDescSegments,
  buildCancelDescText,
  buildMarginDescSegments,
  buildMarginDescText,
  buildCloseDescSegments,
  buildCloseDescText,
} from "../lib/configDesc.ts";
import type {
  AmendConfigItem,
  CancelConfigItem,
  MarginConfigItem,
...
export class StrategyEngine {
  ...
  static async closePositionAdvanced(params: {
    ...
    const ordType = params.priceType === 'limit' ? 'limit' : params.priceType === 'chase' ? 'chase' : 'market';
    ...
    if (params.priceType === 'chase' && params.chaseVal != null) {
      orderParams.chaseVal = params.chaseVal;
    }
  ...
  static async reversePosition(params: {
    ...
    const isBinanceService = ts.exchangeType === "BINANCE";
    const isRequestOk = (res: unknown): boolean => {
      const r = res as { code?: string | number } | null | undefined;
      if (isBinanceService) return r?.code !== undefined && String(r.code) === "0";
      return r?.code === "0";
    };
