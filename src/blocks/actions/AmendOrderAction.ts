      ...
      for (let i = 0; i < ordersToProcess.length; i += BATCH_SIZE) {
        ...
        batch.forEach((order: OrderWithAlgo, batchIdx: number) => {
          ...
          if (amendOrderType === 'trigger') {
            ...
            const triggerIncrementValue = parseFloat(triggerIncrementStr.replace('%', '')) / (isPercentTrigger ? 100 : 1);
            ...
            if (isOkxMoveStop && moveStopAlgo) {
              ...
              if (slAlgo) {
                const slIncrementStr = config.sl_ord_px_increment || "-1";
                const slIncrementValue = parseFloat(slIncrementStr.replace('%', '')) / (slIncrementStr.includes('%') ? 100 : 1);
                const isSlDisabled = Math.abs(slIncrementValue + 1) < 1e-10 || Math.abs(slIncrementValue + 0.01) < 1e-10;
                if (!isSlDisabled) {
                  ...
                  let newSlTriggerPx: number;
                  let newSlOrdPx: number;
                  if (slIncrementStr.includes('%')) {
                    const slPercent = isBuy ? -slIncrementValue : slIncrementValue;
                    newSlTriggerPx = parseFloat((baseTriggerPx * (1 + slPercent)).toFixed(8));
                    newSlOrdPx = parseFloat((baseTriggerPx * (1 + slPercent)).toFixed(8));
                  } else {
                    const slDiff = isBuy ? -slIncrementValue : slIncrementValue;
                    newSlTriggerPx = parseFloat((baseTriggerPx + slDiff).toFixed(8));
                    newSlOrdPx = parseFloat((baseTriggerPx + slDiff).toFixed(8));
                  }
              ...
              const tpIncrementValue = parseFloat(tpIncrementStr.replace('%', '')) / (isPercentTp ? 100 : 1);
              const isPercentSl = slIncrementStr.includes('%');
              const slIncrementValue = parseFloat(slIncrementStr.replace('%', '')) / (isPercentSl ? 100 : 1);
              const isTpDisabled = Math.abs(tpIncrementValue + 1) < 1e-10 || Math.abs(tpIncrementValue + 0.01) < 1e-10;
              const isSlDisabled = Math.abs(slIncrementValue + 1) < 1e-10 || Math.abs(slIncrementValue + 0.01) < 1e-10;
              ...
              if (tpAlgo && !isTpDisabled) {
                ...
                let newTpTriggerPx: number;
                let newTpOrdPx: number;
                
                if (isPercentTp) {
                  const tpPercent = isBuy ? tpIncrementValue : -tpIncrementValue;
                  newTpTriggerPx = parseFloat((baseTriggerPx * (1 + tpPercent)).toFixed(8));
                  newTpOrdPx = parseFloat((baseTriggerPx * (1 + tpPercent)).toFixed(8));
                } else {
                  const tpDiff = isBuy ? tpIncrementValue : -tpIncrementValue;
                  newTpTriggerPx = parseFloat((baseTriggerPx + tpDiff).toFixed(8));
                  newTpOrdPx = parseFloat((baseTriggerPx + tpDiff).toFixed(8));
                }
            ...
            const parseBinanceIncrement = (raw: string | undefined): number => {
              if (raw === undefined || raw === null) return NaN;
              const s = String(raw).trim();
              if (s === '') return NaN;
              const val = parseFloat(s.replace('%', '')) / (s.includes('%') ? 100 : 1);
              return Number.isNaN(val) ? NaN : val;
            };
            const tpIncrementForBinance = parseBinanceIncrement(tpIncrementStr);
            const slIncrementForBinance = parseBinanceIncrement(slIncrementStr);
            const isTpBinanceDisabled = Number.isNaN(tpIncrementForBinance) || Math.abs(tpIncrementForBinance + 1) < 1e-10 || Math.abs(tpIncrementForBinance + 0.01) < 1e-10;
            const isSlBinanceDisabled = Number.isNaN(slIncrementForBinance) || Math.abs(slIncrementForBinance + 1) < 1e-10 || Math.abs(slIncrementForBinance + 0.01) < 1e-10;
            ...
            if (existingAttach && !shouldSkipAttach) {
              ...
              if (isOkxMoveStop) {
                ...
                if (!isSlDisabled) {
                  ...
                  let newSlTriggerPx: number;
                  if (slIncrementStr.includes('%')) {
                    const percentValue = slIncrementValue;
                    newSlTriggerPx = parseFloat((basePx * (1 + (isBuy ? -percentValue : percentValue))).toFixed(8));
                  } else {
                    const slDiff = isBuy ? -slIncrementValue : slIncrementValue;
                    newSlTriggerPx = parseFloat((basePx + slDiff).toFixed(8));
                  }
                  const newSlOrdPx = newSlTriggerPx;
                  attachedAlgo.newSlTriggerPx = newSlTriggerPx.toString();
                  attachedAlgo.newSlOrdPx = newSlOrdPx.toString();
                ...
                const tpIncrementStr = config.tp_ord_px_increment || "-1";
                const slIncrementStr = config.sl_ord_px_increment || "-1";
                const tpIncrementValue = parseFloat(tpIncrementStr.replace('%', '')) / (tpIncrementStr.includes('%') ? 100 : 1);
                const slIncrementValue = parseFloat(slIncrementStr.replace('%', '')) / (slIncrementStr.includes('%') ? 100 : 1);
                const isTpDisabled = Math.abs(tpIncrementValue + 1) < 1e-10 || Math.abs(tpIncrementValue + 0.01) < 1e-10;
                const isSlDisabled = Math.abs(slIncrementValue + 1) < 1e-10 || Math.abs(slIncrementValue + 0.01) < 1e-10;
                ...
                if (!isTpDisabled) {
                  const basePx = AmendOrderAction.applyIncrement(originalPx, pxIncrementStr);
                  let newTpTriggerPx: number;
                  if (tpIncrementStr.includes('%')) {
                    const percentValue = tpIncrementValue;
                    newTpTriggerPx = parseFloat((basePx * (1 + (isBuy ? percentValue : -percentValue))).toFixed(8));
                  } else {
                    const tpDiff = isBuy ? tpIncrementValue : -tpIncrementValue;
                    newTpTriggerPx = parseFloat((basePx + tpDiff).toFixed(8));
                  }
                  const newTpOrdPx = newTpTriggerPx;
            ...
            const tpIncrementForBinance = parseFloat((config.tp_ord_px_increment || "-1").replace('%', '')) / ((config.tp_ord_px_increment || "-1").includes('%') ? 100 : 1);
            const slIncrementForBinance = parseFloat((config.sl_ord_px_increment || "-1").replace('%', '')) / ((config.sl_ord_px_increment || "-1").includes('%') ? 100 : 1);
            const isTpBinanceDisabled = Math.abs(tpIncrementForBinance + 1) < 1e-10 || Math.abs(tpIncrementForBinance + 0.01) < 1e-10;
            const isSlBinanceDisabled = Math.abs(slIncrementForBinance + 1) < 1e-10 || Math.abs(slIncrementForBinance + 0.01) < 1e-10;
