...
import { ClosePositionActionConfig } from "../../types/blocks.ts";
...
export class ClosePositionAction {
  ...
  static async execute(config: ClosePositionActionConfig, tradeService: OKXTradeService, shouldContinue?: () => boolean) {
    const t0 = Date.now();
    const accountIdx = tradeService.accountIdx;
    const instId = config.inst_id || config.close_inst_id || "";
    const testMode = config.close_test_mode === true || config.test_mode === true;
    const configId = config.id || '';
    LogService.info('close-perf', `[t+0ms] ClosePositionAction.execute 开始 accountIdx=${accountIdx} instId=${instId || '(全部)'} testMode=${testMode}`);
    ...
    try {
      ...
      if (!testMode) {
        try {
          ...
          LogService.info('close-perf', `[t+${Date.now() - t0}ms] monitor.syncPositions 完成 (${Date.now() - tSync0}ms)`);
        } catch (err: unknown) {
          const syncElapsed = Date.now() - t0;
          LogService.info('close-perf', `[t+${syncElapsed}ms] monitor.syncPositions 失败(${syncElapsed}ms) ${err instanceof Error ? err.message : String(err)}`);
  ...
  private static formatPositionDetails(position: Position): string {
    ...
    return `交易对=${instId}, 方向=${posSideText}, 仓位模式=${mgnModeText}, 持仓数量=${pos}, 开仓均价=${avgPx}, 未实现盈亏=${upl} (${uplRatio}%)`;
  }
}
