...
import { evaluateFormula, formulaNeedsVar, isFormula } from "../../lib/priceFormula.ts";
import { fetchMarketPrice } from "../../lib/marketPrice.ts";
...
export class PlaceOrderAction {
  private static async resolvePrice(rawValue: string, instId: string, extraVars?: { o?: number; n?: number; s?: number; marketPrice?: number }): Promise<number | null> {
    ...
    if (isOpenPosition) {
      if (config.isMoveStop) {
        ...
        if (config.activePxVal > 0) {
          algoOrd.activePx = config.activePxVal.toFixed(8);
        }
        if (config.slMargin !== -1) {
          const slPrice = PlaceOrderAction.calcTpSlPrice(basePrice, config.slMargin, config.slIsPercent, config.side, false);
          if (slPrice > 0) algoOrd.slTriggerPx = slPrice.toFixed(8);
        }
        ...
        if (config.tpMargin !== -1) {
          const tpPrice = PlaceOrderAction.calcTpSlPrice(basePrice, config.tpMargin, config.tpIsPercent, config.side, true);
          if (tpPrice > 0) algoOrd.tpTriggerPx = tpPrice.toFixed(8);
        }
        if (config.slMargin !== -1) {
          const slPrice = PlaceOrderAction.calcTpSlPrice(basePrice, config.slMargin, config.slIsPercent, config.side, false);
          if (slPrice > 0) algoOrd.slTriggerPx = slPrice.toFixed(8);
  ...
  private static async placeTriggerOrder(params: {
    ...
    tradeService: OKXTradeService; cachedPositionSize: number | null; cachedMarketPrice: number | null;
    actualNumOrders: number; posSide: string;
    ...
    const { i, ordType, instId, side, tdMode, effectivePosSide, contractSize, requestKey,
      ...
      tradeService, cachedPositionSize, cachedMarketPrice, actualNumOrders, posSide } = params;

    const triggerPx = parseFloat((curFirstOrderPrice + priceOffset).toFixed(8));
    const curActivePx = (activePxNeedsO || activePxNeedsN)
      ? (await PlaceOrderAction.resolvePrice(rawActivePx, instId, { o: triggerPx, n: i, s: cachedPositionSize ?? undefined, marketPrice: cachedMarketPrice !== null ? cachedMarketPrice : undefined })) ?? -1
      : activePxVal;
    ...
    if (isSwapOrFutures) {
      attachAlgoOrds = PlaceOrderAction.buildAttachAlgoOrds(
        { side, posSide, isMoveStop, tpMargin: takeProfitMargin, slMargin: stopLossMargin, tpIsPercent, slIsPercent, callbackRatioVal, callbackSpreadVal, activePxVal: curActivePx },
        triggerPx,
        requestKey,
        i
      );
  ...
  private static async placeConditionalOrder(params: {
    ...
    skipDuplicateOrders: boolean; existingPrices: number[];
    testMode: boolean; configId?: string;
    tradeService: OKXTradeService; actualNumOrders: number;
  }): Promise<{ success: boolean; placed: boolean }> {
    const { i, ordType, instId, side, tdMode, effectivePosSide, contractSize, requestKey,
      firstTpPrice, firstSlPrice, priceOffset, marketBasePrice,
      skipDuplicateOrders, existingPrices, testMode, configId,
      tradeService, actualNumOrders } = params;
    ...
    if (ordType === 'conditional' && marketBasePrice !== null) {
      const triggerPx = parseFloat((firstTpPrice + priceOffset).toFixed(8));
      ...
      const tpTriggerPxVal = tpDisabled ? null : parseFloat((firstTpPrice + priceOffset).toFixed(8));
      const slTriggerPxVal = slDisabled ? null : parseFloat((firstSlPrice + priceOffset).toFixed(8));
  ...
  private static async placeChaseOrder(params: {
    ...
    const { i, ordType, instId, side, tdMode, effectivePosSide, contractSize, requestKey,
      ...
      testMode, configId, tradeService, actualNumOrders } = params;
  ...
  private static async placeMoveOrderStopOrder(params: {
    ...
    const currentActivePx = activePxVal > 0 ? parseFloat((activePxVal + priceOffset).toFixed(8)) : null;
  ...
  static async execute(config: PlaceOrderActionConfig, tradeService: OKXTradeService, numOrders?: number, configId?: string, appConfig?: AppConfig) {
    ...
    const tpMargin = PlaceOrderAction.parseMarginValue(config.take_profit_margin || "-1");
    const slMargin = PlaceOrderAction.parseMarginValue(config.stop_loss_margin || "-1");
    const takeProfitMargin = tpMargin.value;
    const stopLossMargin = slMargin.value;
    const tpIsPercent = tpMargin.isPercent;
    const slIsPercent = slMargin.isPercent;
    ...
    if (!activePxNeedsO && !activePxNeedsN) {
      activePxVal = isFormula(rawActivePx)
    ...
    try {
      ...
      for (let i = 0; i < actualNumOrders; i++) {
        ...
        if (firstOrderPriceNeedsN) {
          curFirstOrderPrice = (await PlaceOrderAction.resolvePrice(rawFirstOrderPrice, instId, { n, s: cachedPositionSize ?? undefined, marketPrice: cachedMarketPrice !== null ? cachedMarketPrice : undefined })) ?? 0;
        ...
        let contractSize = "1";
        if (isSizeFormula) {
          const triggerPx = curFirstOrderPrice + priceOffset;
          const vars = { ...cachedVars, n, N: n, o: triggerPx, O: triggerPx };
        ...
        if (ordType === 'trigger') {
          const r = await PlaceOrderAction.placeTriggerOrder({
            ...
            takeProfitMargin, stopLossMargin, tpIsPercent, slIsPercent,
            skipDuplicateOrders, existingPrices, testMode, configId,
            tradeService, cachedPositionSize, cachedMarketPrice, actualNumOrders, posSide,
          });
          ...
          const r = await PlaceOrderAction.placeChaseOrder({
            i, ordType, instId, side, tdMode, effectivePosSide, contractSize, requestKey,
            chaseVal, priceOffset, skipDuplicateOrders, existingPrices,
            testMode, configId, tradeService, actualNumOrders,
          });
          if (r.success) successCount++;
          if (r.placed) actualPlacedCount++;
          if (!r.success && r.placed) failCount++;
          ...
          const px = parseFloat((basePrice + effectiveOffset).toFixed(8));
          const curActivePx = (activePxNeedsO || activePxNeedsN)
            ? (await PlaceOrderAction.resolvePrice(rawActivePx, instId, { o: px, n, s: cachedPositionSize ?? undefined, marketPrice: cachedMarketPrice !== null ? cachedMarketPrice : undefined })) ?? -1
            : activePxVal;
