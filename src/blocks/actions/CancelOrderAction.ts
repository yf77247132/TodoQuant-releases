...
export class CancelOrderAction {
  ...
  static async execute(config: CancelOrderActionConfig, tradeService: OKXTradeService, shouldContinue?: () => boolean) {
    ...
    try {
      ...
      if (!testMode && totalSuccess > 0) {
        const accountId = AccountMonitor.getInstance().getAccountData(accountIdx)?.accountId || '';
  ...
  private static async processBatch(
    ...
    const pending51412Confirm: Promise<void>[] = [];
    const wrapConfirmCancelTimeout = (order: UnifiedOrder, id: string) => {
      pending51412Confirm.push(new Promise<void>((resolve) => {
        CancelOrderAction.confirmCancelTimeout(order, accountIdx, isAlgo, (success, detail) => {
          if (success) {
            successCount++;
            ...
            LogService.logKey("cancel", 'order.cancel.ok', {
              num: currentTotal,
              ordId: id,
              detail: `${CancelOrderAction.formatOrderDetails(id, order)}${detail}`,
      ...
      try {
        ...
        if (res && (res.code === "0" || res.code === "2")) {
          batch.forEach((order, idx) => {
            ...
            } else if (String(sCode) === '51412') {
              wrapConfirmCancelTimeout(order, id);
            } else {
              failCount++;
              const currentTotal = startNum + successCount + failCount;
              LogService.logKey("cancel", 'order.cancel.fail', {
          ...
          batch.forEach((order, idx) => {
            const id = batchIds[idx];
            const itemData = res?.data?.[idx] || {};
            if (String(itemData.sCode) === '51412') {
              wrapConfirmCancelTimeout(order, id);
              return;
            }
            ...
            for (let w = 0; w < weight; w++) {
              failCount++;
              const currentTotal = startNum + successCount + failCount;
              LogService.logKey("cancel", 'order.cancel.fail', { num: currentTotal, reason: formatApiError(res, idx), params: buildCancelFailParams(id, order) }, 'warn', configId);
            }
          });
        }
      } catch (err: unknown) {
    ...
    await Promise.all(pending51412Confirm);

    return { success: successCount, fail: failCount };
  }

  private static confirmCancelTimeout(
    order: UnifiedOrder,
    accountIdx: number,
    isAlgo: boolean,
    onResolved: (success: boolean, detail: string) => void
  ): void {
    const ordIdKey = order.ordId || String(order.algoId || '');
    const checkCache = () => {
      const ordersCache = globalThis.ORDERS_CACHE?.get(accountIdx);
      const allCached = ordersCache?.data || [];
      return allCached.some((o: any) => (o.ordId || o.algoId) === ordIdKey);
    };
    if (!ordIdKey) {
      onResolved(false, 'OKX 51412 超时且无有效订单ID');
      return;
    }
    if (!checkCache()) {
      onResolved(true, '（OKX 51412 超时，缓存已清除，视为成功）');
      return;
    }
    ...
    const pollTimer = setInterval(() => {
      ...
      if (!checkCache() || pollCount >= MAX_RETRIES) {
        ...
        if (!checkCache()) {
          onResolved(true, `（OKX 51412 超时，WS 轮询 ${pollCount}/${MAX_RETRIES} 确认已取消）`);
        } else {
          onResolved(false, `OKX 51412 超时，WS 轮询 ${MAX_RETRIES} 次后缓存仍有 → 确认失败`);
        }
      }
    }, 500);
  }
