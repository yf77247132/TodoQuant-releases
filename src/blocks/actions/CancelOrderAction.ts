...
import { isPendingNewState, isAlgoOrderType } from "../../lib/orderStates.ts";
...
export class CancelOrderAction {
  ...
  private static readonly ALGO_TYPE_SET = new Set(['trigger', 'conditional', 'oco', 'chase', 'move_order_stop']);
  ...
  private static filterOrderList(
  ...
  private static getFilteredOrders(
    ...
    return CancelOrderAction.filterOrderList(allOrders, instId, orderTypes, isBinance);
  }
  ...
  static async execute(config: CancelOrderActionConfig, tradeService: OKXTradeService, shouldContinue?: () => boolean) {
    ...
    LogService.info('cancel-perf', `[t+0ms] CancelOrderAction.execute 开始 accountIdx=${accountIdx} instId=${instId || '(全部)'} types=${orderTypes.join(',')} testMode=${testMode}`);
    ...
    try {
      ...
      LogService.info('cancel-perf', `[t+${tGet0 - t0}ms] getFilteredOrders 完成 (${tGet0 - t0}ms) 得到 ${orders.length} 笔`);

      if (orders.length === 0) {
        try {
          const algoTypes = orderTypes.filter(t => CancelOrderAction.ALGO_TYPE_SET.has(t));
          const hasRegular = orderTypes.some(t => !CancelOrderAction.ALGO_TYPE_SET.has(t));
          const fetched: UnifiedOrder[] = [];

          for (const algoType of algoTypes) {
            if (typeof (tradeService as any).getPendingAlgoOrders !== 'function') continue;
            const tA0 = Date.now();
            const algoOrders = await (tradeService as any).getPendingAlgoOrders(instId || undefined, algoType);
            LogService.info('cancel-perf', `[t+${Date.now() - t0}ms] getPendingAlgoOrders(${algoType}) 补偿完成 (${Date.now() - tA0}ms) 得到 ${algoOrders?.length || 0} 笔`);
            for (const o of (algoOrders || [])) {
              fetched.push({ ...o, algoOrdType: algoType } as UnifiedOrder);
            }
          }

          if (hasRegular && typeof (tradeService as any).getOpenOrders === 'function') {
            const tN0 = Date.now();
            const normalOrders = await (tradeService as any).getOpenOrders(instId || undefined);
            LogService.info('cancel-perf', `[t+${Date.now() - t0}ms] getOpenOrders 补偿完成 (${Date.now() - tN0}ms) 得到 ${normalOrders?.length || 0} 笔`);
            for (const o of (normalOrders || [])) {
              fetched.push(o as UnifiedOrder);
            }
          }

          orders = CancelOrderAction.filterOrderList(fetched, instId, orderTypes, isBinance);
          LogService.info('cancel-perf', `[t+${Date.now() - t0}ms] 补偿过滤后得到 ${orders.length} 笔`);
        } catch (err: unknown) {
      ...
      const normalOrders = finalOrders.filter(o => !isAlgoOrderType(o));
      const algoOrders = finalOrders.filter(o => isAlgoOrderType(o));
  ...
  private static async processBatch(
    ...
    for (let i = 0; i < orders.length; i += BATCH_SIZE) {
      ...
      const batchIds = batch.map(o => isAlgo ? o.algoId : o.ordId);
      ...
      try {
        const tOkx0 = Date.now();
        const okxApi = isAlgo ? 'cancelBatchAlgoOrders' : 'cancelBatchOrders';
        const totalBatches = Math.ceil(orders.length / BATCH_SIZE);
        const batchNum = Math.floor(i / BATCH_SIZE) + 1;
        LogService.info('cancel-perf', `[t+${Date.now() - t0}ms] 调用 OKX ${okxApi} 第 ${batchNum}/${totalBatches} 批 size=${batch.length}`);
        const res = isAlgo
          ? await tradeService.cancelBatchAlgoOrders(batchParams as any)
          : await tradeService.cancelBatchOrders(batchParams);
        LogService.info('cancel-perf', `[t+${Date.now() - t0}ms] OKX ${okxApi} 返回 code=${res?.code} (${Date.now() - tOkx0}ms)`);
        ...
        if (res && (res.code === "0" || res.code === "2")) {
          batch.forEach((order, idx) => {
            ...
            if (sCode === "0") {
              ...
              const sCodeStr = String(sCode);
              if (sCodeStr === '51412') {
                const ordIdKey = order.ordId || String(order.algoId || '');
                if (ordIdKey) {
                  const checkCache = () => {
                    const ordersCache = (globalThis as any)?.ORDERS_CACHE?.get?.(accountIdx);
                    const allCached = ordersCache?.data || [];
                    return allCached.some((o: any) => (o.ordId || o.algoId) === ordIdKey);
                  };
                  if (!checkCache()) {
                    successCount++;
                    const currentTotal = startNum + successCount + failCount;
                    LogService.logKey("cancel", 'order.cancel.ok', {
                      num: currentTotal,
                      ordId: id,
                      detail: `${CancelOrderAction.formatOrderDetails(id, order)}（OKX 51412 超时，缓存已清除，视为成功）`,
                      type: CancelOrderAction.getOrderTypeLabel(order),
                      instId: String(order.instId || ''),
                    }, 'info', configId);
                    return;
                  }
                  const MAX_RETRIES = isAlgo ? 12 : 6;
                  let pollCount = 0;
                  const pollTimer = setInterval(() => {
                    pollCount++;
                    if (!checkCache() || pollCount >= MAX_RETRIES) {
                      ...
                      if (!checkCache()) {
                        ...
                        LogService.logKey("cancel", 'order.cancel.ok', {
                          num: currentTotal,
                          ordId: id,
                          detail: `${CancelOrderAction.formatOrderDetails(id, order)}（OKX 51412 超时，WS 轮询 ${pollCount}/${MAX_RETRIES} 确认已取消）`,
                          type: CancelOrderAction.getOrderTypeLabel(order),
                          instId: String(order.instId || ''),
                        }, 'info', configId);
                      } else {
                        ...
                        LogService.logKey("cancel", 'order.cancel.fail', {
                          num: currentTotal,
                          reason: `OKX 51412 超时，WS 轮询 ${MAX_RETRIES} 次后缓存仍有 → 确认失败`,
