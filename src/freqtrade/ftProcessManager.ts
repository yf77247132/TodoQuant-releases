...
import { killProcessTree } from '../lib/processUtils.ts';
import { generateStrategyFile, computeLiveDownloadDays } from './strategyGenerator.ts';
import type {
  FtProcessSpec,
  FtProcessInfo,
  FtDetectionResult,
...
const MAX_BACKTEST_RESULTS = 5;

function cleanupOldBacktestResults(): void {
  try {
    const btDir = path.join(FT_USER_DATA_DIR, 'backtest_results');
    if (!fs.existsSync(btDir)) return;

    const files = fs.readdirSync(btDir)
      .filter(f => f.endsWith('.zip'))
      .map(f => ({ name: f, path: path.join(btDir, f) }))
      .filter(f => fs.existsSync(f.path))
      .sort((a, b) => {
        try { return fs.statSync(b.path).mtimeMs - fs.statSync(a.path).mtimeMs; } catch { return 0; }
      });

    if (files.length <= MAX_BACKTEST_RESULTS) return;

    const toDelete = files.slice(MAX_BACKTEST_RESULTS);
    for (const f of toDelete) {
      try {
        fs.unlinkSync(f.path);
        const metaPath = f.path.replace(/\.zip$/, '.meta.json');
        if (fs.existsSync(metaPath)) fs.unlinkSync(metaPath);
      } catch {  }
    }
    LogService.logKey('freqtrade', 'ft.backtestResultsCleaned', { count: toDelete.length, kept: MAX_BACKTEST_RESULTS });
  } catch {  }
}

function cleanupPycache(): void {
  try {
    const cacheDir = path.join(FT_USER_DATA_DIR, 'strategies', '__pycache__');
    if (!fs.existsSync(cacheDir)) return;

    const files = fs.readdirSync(cacheDir).filter(f => f.endsWith('.pyc'));
    for (const f of files) {
      try { fs.unlinkSync(path.join(cacheDir, f)); } catch {  }
    }
    if (files.length > 0) {
      LogService.logKey('freqtrade', 'ft.pycacheCleaned', { count: files.length });
    }
  } catch {  }
}
...
export function writeTempConfig(prefix: string, config: object): string {
...
export function removeTempConfig(configPath: string): void {
  try {
...
class FtProcessManager {
  ...
  private async killOrphanFreqtradePorts(): Promise<void> {
    ...
    try {
      ...
      for (const line of out.split('\n')) {
        ...
        const pid = match[2];
        if (port >= 8080 && port <= 8099 && pid && pid !== '0' && pid !== String(process.pid) && !killed.has(pid)) {
          try {
  ...
  private getFreqtradeCandidates(): string[] {
    ...
    try {
      // @ts-expect-error — Electron-only global（@types/node 无此属性）
  ...
  async startLiveStrategy(
    ...
    this.pendingStarts.add(spec.id);
    ...
    try {
      ...
      this.pendingStarts.delete(spec.id);
      return { ok: false, error: `策略文件写入失败: ${err.message}` };
    }

    if (spec.pairWhitelist.length > 0) {
      const downloadDays = computeLiveDownloadDays(strategySpec);
      try {
        const { ftBacktestService } = await import('./ftBacktestService.ts');
    ...
    if (!apiPort) {
      ...
      while (usedPorts.has(apiPort) && apiPort < FT_API_PORT_MAX) apiPort++;
      if (usedPorts.has(apiPort) && apiPort >= FT_API_PORT_MAX) {
        const msg = `FT API 端口池耗尽（${FT_API_PORT_BASE}-${FT_API_PORT_MAX}），请重启应用或减少同时运行的策略数`;
        LogService.error('freqtrade', msg);
        this.pendingStarts.delete(spec.id);
        return { ok: false, error: msg };
      }
    ...
    const startPromise = new Promise<FtStartResult>((resolve) => {
        ...
        child.stdout?.on('data', (d: Buffer) => {
          ...
          if (!resolved.value && (output.includes('Bot started') || output.includes('dry_run'))) {
            ...
            if (this.cancelledStartIds.has(spec.id)) {
              ...
              this.cancelledStartIds.delete(spec.id);
              resolve({ ok: false, error: '策略已在启动期间被停止' });
              return;
            }
            this.cancelledStartIds.delete(spec.id);
  ...
  async stopStrategy(strategyId: string): Promise<void> {
    ...
    if (!info) {
      if (this.pendingStarts.has(strategyId)) {
        LogService.logKey('freqtrade', 'ft.stopDuringStart', { id: strategyId });
      } else {
    ...
    await this.gracefulStopFt(strategyId);
  ...
  async stopAll(): Promise<void> {
    ...
    await Promise.all(Array.from(this.processes.keys()).map(id => this.gracefulStopFt(id)));
  ...
  private async gracefulStopFt(strategyId: string): Promise<void> {
    const apiPort = allocatedPorts.get(strategyId);
    if (!apiPort) return;
    try {
      const auth = Buffer.from('okts:okts').toString('base64');
      await fetch(`http://127.0.0.1:${apiPort}/api/v1/stop`, {
        method: 'POST',
        headers: { 'Authorization': `Basic ${auth}` },
        signal: AbortSignal.timeout(2000),
      });
