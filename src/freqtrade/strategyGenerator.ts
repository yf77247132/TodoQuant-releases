...
export function jsDayToPandasDay(d: number): number {
  return (d + 6) % 7;
}
...
export function timeframeToMinutes(tf: string): number {
  ...
  const mult = m[2] === 'm' ? 1 : m[2] === 'h' ? 60 : m[2] === 'd' ? 1440 : m[2] === 'w' ? 10080 : 43200;
  return n * mult;
}

export function computeLiveDownloadDays(spec: FreqtradeStrategySpec): number {
  const conditions = [...(spec.entry?.children ?? []), ...(spec.exit?.children ?? [])];
  const candles = computeStartupCandles(conditions);
  const tfMinutes = timeframeToMinutes(spec.timeframe);
  if (candles <= 0 || tfMinutes <= 0) return 7;
  const days = Math.ceil((candles * 1.5 * tfMinutes) / 1440);
  return Math.min(Math.max(days, 7), 1825);
}
...
function generateTimeWindowCheck(timeWindows: TimeWindowSpec[]): string {
  ...
  for (const tw of timeWindows) {
    ...
    if (tw.days.length > 0) {
      const daysPy = `[${tw.days.map(jsDayToPandasDay).join(', ')}]`;
...
export function generateStrategyFile(spec: FreqtradeStrategySpec, mainPair?: string, mode: 'live' | 'backtest' = 'live'): string {
  ...
  if (hasExitConditions) {
    ...
    parts.push(`${IND2}] = 1`);
    if (isShortOnly) {
      parts.push(`${IND2}#做空出场（与做多共用出场条件，平空头仓位）`);
      parts.push(`${IND2}dataframe.loc[`);
      parts.push(exitConditions);
      parts.push(`${IND3},`);
      parts.push(`${IND3}"exit_short"`);
      parts.push(`${IND2}] = 1`);
    }
  } else {
    parts.push(`${IND2}pass`);
