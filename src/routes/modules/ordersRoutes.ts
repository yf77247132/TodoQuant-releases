...
export function registerOrdersRoutes(app: Application, config: ApiRoutesConfig): void {
  ...
  app.post('/api/position/close', asyncHandler(async (req, res) => {
    ...
    if (!resolved) return;
    LogService.info('close-perf', `[t+0ms] 收到单笔平仓请求 account=${resolved.account.id} instId=${instId} posSide=${posSide || ''}`);

    const tClose0 = Date.now();
    const result = await StrategyEngine.closePosition(resolved.account.id, instId, mgnMode, posSide, ccy);
    LogService.info('close-perf', `[t+${Date.now() - t0}ms] StrategyEngine.closePosition 返回 code=${result?.code} (${Date.now() - tClose0}ms)`);
    ...
    if (result.code === '0') {
      ...
      LogService.info('close-perf', `[t+${Date.now() - t0}ms] triggerSync 已触发`);
    }
    LogService.info('close-perf', `[t+${Date.now() - t0}ms] 单笔平仓完成 总耗时=${Date.now() - t0}ms`);
  ...
  app.post('/api/position/close-advanced', asyncHandler(async (req, res) => {
    ...
    LogService.info('close-perf', `[t+0ms] 收到高级平仓请求 account=${resolved.account.id} instId=${instId} posSide=${posSide || ''} priceType=${priceType} sz=${sz || '100%'}`);
    ...
    LogService.info('close-perf', `[t+${Date.now() - t0}ms] StrategyEngine.closePositionAdvanced 返回 code=${result?.code} (${Date.now() - tClose0}ms)`);
    ...
    LogService.info('close-perf', `[t+${Date.now() - t0}ms] 高级平仓完成 总耗时=${Date.now() - t0}ms`);
  ...
  app.post('/api/position/batch-close', asyncHandler(async (req, res) => {
    ...
    LogService.info('close-perf', `[t+0ms] 收到批量平仓请求 ${positions.length} 笔`);
    ...
    for (const [i, pos] of positions.entries()) {
      ...
      try {
        ...
        const elapsed = Date.now() - tClose0;
        if (result?.code === '0') {
          success++;
          details.push({ instId, posSide: posSide || '', success: true });
          syncedAccountIds.add(accountId);
          LogService.info('close-perf', `[t+${Date.now() - t0}ms] 第 ${i + 1}/${positions.length} 笔 ${instId} 平仓成功 (${elapsed}ms)`);
        } else {
          fail++;
          details.push({ instId, posSide: posSide || '', success: false, error: result?.msg || '平仓失败' });
          LogService.info('close-perf', `[t+${Date.now() - t0}ms] 第 ${i + 1}/${positions.length} 笔 ${instId} 平仓失败 (${elapsed}ms) code=${result?.code} msg=${result?.msg}`);
        }
      } catch (e: unknown) {
        ...
        LogService.info('close-perf', `[t+${Date.now() - t0}ms] 第 ${i + 1}/${positions.length} 笔 ${instId} 异常 (${Date.now() - tClose0}ms) ${toError(e).message}`);
      }
    ...
    LogService.info('close-perf', `[t+${Date.now() - t0}ms] 批量平仓完成 总耗时=${Date.now() - t0}ms 成功${success}, 失败${fail}`);
