...
import { asyncHandler, resolveAccountFromRequest, sendError, toError } from './shared.ts';
...
import { getHistoryCache, setHistoryCache, isRecordStale, INVALIDATED_WINDOW_MS, STALE_WINDOW_MS, STALE_SHORT_TTL_MS, invalidatePositionsHistoryCache, invalidateOrdersHistoryCache } from '../../services/historyCacheService.ts';
...
export function registerOrdersRoutes(app: Application, config: ApiRoutesConfig): void {
  ...
  app.post('/api/orders/history', asyncHandler(async (req, res) => {
    ...
    let rawOrders = await (tradeService as any).getHistoryOrders?.(instType || 'SWAP', instId, after, limit);
    if (!Array.isArray(rawOrders)) {
      return res.json({ ok: true, data: [] });
    }

    if (isRecordStale(accountId, rawOrders, INVALIDATED_WINDOW_MS, 'orders')) {
      await sleep(3000);
      const retried = await (tradeService as any).getHistoryOrders?.(instType || 'SWAP', instId, after, limit);
      if (Array.isArray(retried)) rawOrders = retried;
    }
    ...
    const stale = isRecordStale(accountId, historyOrders, STALE_WINDOW_MS, 'orders');
    const payload = { ok: true, data: historyOrders, stale };
    setHistoryCache(cacheKey, payload, stale ? STALE_SHORT_TTL_MS : undefined);
    res.json(payload);
  }));
  ...
  app.post('/api/orders/algo-history', asyncHandler(async (req, res) => {
    ...
    let rawOrders = await (tradeService as any).getHistoryAlgoOrders?.(instType || 'SWAP', instId, after, limit);
    if (!Array.isArray(rawOrders)) {
      return res.json({ ok: true, data: [] });
    }
    ...
    if (isRecordStale(accountId, rawOrders, INVALIDATED_WINDOW_MS, 'orders')) {
      await sleep(3000);
      const retried = await (tradeService as any).getHistoryAlgoOrders?.(instType || 'SWAP', instId, after, limit);
      if (Array.isArray(retried)) rawOrders = retried;
    }
    ...
    const stale = isRecordStale(accountId, historyAlgoOrders, STALE_WINDOW_MS, 'orders');
    const payload = { ok: true, data: historyAlgoOrders, stale };
    setHistoryCache(algoCacheKey, payload, stale ? STALE_SHORT_TTL_MS : undefined);
    res.json(payload);
  }));
  ...
  app.post('/api/order/cancel', asyncHandler(async (req, res) => {
    ...
    const cancelOk = result?.code === '0';
    res.status(cancelOk ? 200 : 502).json({ ok: cancelOk, data: result, error: errMsg, okxCode: result?.code });
  }));
  ...
  app.post('/api/position/close', asyncHandler(async (req, res) => {
    ...
    const closeOk = result?.code === '0';
    res.status(closeOk ? 200 : 502).json({ ok: closeOk, data: result });
  }));

  app.post('/api/position/close-advanced', asyncHandler(async (req, res) => {
    const { instId, mgnMode, posSide, ccy, priceType, px, chaseVal, sz, posQty } = req.body;
    const resolved = resolveAccountFromRequest(req, res);
    if (!resolved) return;
    if (!priceType || !['market', 'limit', 'chase'].includes(priceType)) {
      return res.status(400).json({ ok: false, error: 'priceType 必须为 market、limit 或 chase' });
    }
  ...
  app.post('/api/position/open-advanced', asyncHandler(async (req, res) => {
    ...
    res.status(isSuccess ? 200 : 502).json({ ok: isSuccess, data: result });
  }));
  ...
  app.post('/api/position/reverse', asyncHandler(async (req, res) => {
    ...
    let result: { closeResult: unknown; openResult: unknown };
    try {
      result = await StrategyEngine.reversePosition({
        accountId: resolved.account.id,
        instId,
        mgnMode,
        posSide,
  ...
  app.post('/api/position/batch-close', asyncHandler(async (req, res) => {
    ...
    const batchCloseOk = fail === 0;
    res.status(batchCloseOk ? 200 : 502).json({ ok: batchCloseOk, success, fail, details });
  }));
  ...
  app.post('/api/order/batch-cancel', asyncHandler(async (req, res) => {
    ...
    for (const [accountId, accountOrders] of grouped) {
      ...
      await runBatched(normalBatchTasks, 1, 150);
      ...
      if (pendingNormalChecks.length > 0) {
        ...
        const cache = config.ordersCache.get(accountIdx);
      ...
      await runBatched(algoBatchTasks, 1, 1500);
  ...
  app.post('/api/positions/history', asyncHandler(async (req: Request, res: Response) => {
    const { accountId, instType, instId, limit, after } = req.body || {};
    if (!accountId) {
      return res.status(400).json({ ok: false, error: '缺少 accountId' });
    }
    const posCacheKey = `positions-history:${accountId}:${instType || 'SWAP'}:${instId || ''}:${limit || 100}:${after || ''}`;
    const forceRefresh = req.body?.refresh === true;
    if (!forceRefresh) {
    ...
    let rawPositions = await (tradeService as any).getHistoryPositions?.(instType || 'SWAP', instId, limit, after);
    if (!Array.isArray(rawPositions)) {
      return res.json({ ok: true, data: [] });
    }

    if (isRecordStale(accountId, rawPositions, INVALIDATED_WINDOW_MS, 'positions')) {
      await sleep(3000);
      const retried = await (tradeService as any).getHistoryPositions?.(instType || 'SWAP', instId, limit, after);
      if (Array.isArray(retried)) rawPositions = retried;
    }
    ...
    const stale = isRecordStale(accountId, historyPositions, STALE_WINDOW_MS, 'positions');
    const payload = { ok: true, data: historyPositions, stale };
    setHistoryCache(posCacheKey, payload, stale ? STALE_SHORT_TTL_MS : undefined);
    res.json(payload);
  }));
}
