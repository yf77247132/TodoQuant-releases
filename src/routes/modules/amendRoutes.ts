...
export function registerAmendRoutes(app: Application): void {
  registerCrudRoutes<AmendConfig>(app, {
    ...
    buildStartConfig: (cfg) => {
      return {
        ...
        tp_ord_px_increment: cfg.tp_ord_px_increment ?? "29",
  ...
  app.post("/api/amend/run", (req, res) => {
    try {
      const cfg = req.body;
      ...
      (async () => {
        try {
          ...
          const { AmendOrderAction } = await import('../../blocks/actions/AmendOrderAction.ts');
          LogService.logKey("amend", 'task.start.begin', { type: '改单' }, 'info', runId);
          await AmendOrderAction.execute(moduleConfig as any, tradeService);
