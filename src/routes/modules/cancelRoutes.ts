...
export function registerCancelRoutes(app: Application): void {
  ...
  app.post("/api/cancel/run", asyncHandler(async (req, res) => {
    const cfg = req.body;
    ...
    try {
      ...
      if (!tradeService) {
        ...
        return res.status(400).json({ ok: false, runId, error: '账户 API 密钥未配置，无法执行撤单' });
      }

      LogService.logKey("cancel", 'task.start.begin', { type: '撤单' }, 'info', runId);
      const summary = await CancelOrderAction.execute(moduleConfig as any, tradeService as any);
