...
export function registerFreqtradeRoutes(app: Application): void {
  ...
  app.post('/api/freqtrade/download-data', asyncHandler(async (req, res) => {
    const { exchange, pairWhitelist, timeframes, days } = req.body || {};
    ...
    if (!days || days <= 0) {
      return res.status(400).json({ ok: false, error: 'days 必须大于 0' });
    }
  ...
  app.post('/api/freqtrade/backtest/save', asyncHandler(async (req, res) => {
    ...
    if (!strategyId || !result) return res.json({ ok: false, error: 'Missing strategyId or result' });
  ...
  app.get('/api/freqtrade/backtest/last/:strategyId', asyncHandler(async (req, res) => {
    ...
    if (!strategyId) return res.json({ ok: false, error: 'Missing strategyId' });
