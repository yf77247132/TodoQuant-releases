...
import { isLoopbackRequest, allowRemoteFtWebhook } from '../../lib/networkUtils.ts';
...
export function registerWebhookRoutes(app: Application): void {
  ...
  app.post('/api/webhook/freqtrade', asyncHandler(async (req: Request, res: Response) => {
    if (!isLoopbackRequest(req) && !allowRemoteFtWebhook()) {
      LogService.logKey('freqtrade', 'webhook.rejected.remote', { addr: req.socket?.remoteAddress || 'unknown' }, 'warn');
      return res.status(403).json({ ok: false, error: 'Forbidden: freqtrade webhook 仅接受本机请求' });
    }
  ...
  function handleTradingViewWebhook(req: any, res: any) {
    ...
    try {
      ...
      if (tvCondition && !expectedSecret) {
        LogService.logKey('diy', 'webhook.noSecret', { strategyId, name: st.name }, 'error', String(st.id));
        return res.status(403).json({ ok: false, error: 'TV webhook secret not configured' });
      }
