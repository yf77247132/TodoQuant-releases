...
export function registerTraderRoutes(app: Application): void {
  ...
  app.post("/api/trader/run", async (req, res) => {
    if (quickRunBusy) {
      LogService.logKey('trader', 'task.run.rejected.busy', {}, 'warn', 'QuickTrade');
      res.status(429).json({ ok: false, msg: '上一次极速下单尚未完成，请稍候' });
      return;
    }
    quickRunBusy = true;
    try {
      ...
      try {
        ...
        if (!tradeService) {
          ...
          res.status(400).json({ ok: false, runId, msg: '未配置 API Key' });
          return;
        }
