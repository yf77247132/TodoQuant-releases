...
export function registerTraderRoutes(app: Application): void {
  ...
  app.post("/api/trader/run", (req, res) => {
    try {
      ...
      (async () => {
        try {
          ...
          await PlaceOrderAction.execute(moduleConfig as any, tradeService);
          LogService.logKey("trader", 'task.complete', { type: 'QuickTrade' }, 'info', runId);
        } catch (err: unknown) {
