...
import { backupTo, closeDb, dbService, reloadDb } from '../../services/dbService.ts';
import { getDiagnosticsSnapshot } from '../../services/diagnosticsService.ts';
import { tunnelManager } from '../../services/tunnelManager.ts';
import { evaluateFormula, isFormula, formatPreview, formulaNeedsVar } from '../../lib/priceFormula.ts';
import { InstrumentService } from '../../services/instrumentService.ts';
import { CoinIconService } from '../../services/coinIconService.ts';
import { LogService } from '../../services/logService.ts';
import { fetchMarketPrice, fetchSodUtc8, fetchFundingRates } from '../../lib/marketPrice.ts';
...
import { setLeverageStatic } from '../../services/okxTradeService.ts';
...
import { getResolvedPort } from '../../lib/portManager.ts';
...
export function registerSystemRoutes(app: Application): void {
  ...
  registerCoinIconSubRoutes(app);
  registerFundingRateSubRoutes(app);
  registerLocaleSubRoutes(app);
  registerTelegramSubRoutes(app);
  registerLeverageSubRoutes(app);
  registerUiPrefsSubRoutes(app);
}

function registerUiPrefsSubRoutes(app: Application): void {
  app.get('/api/ui-prefs/:key', (req, res) => {
    const { key } = req.params;
    if (!/^[a-zA-Z0-9_-]{1,64}$/.test(key)) {
      return res.status(400).json({ error: 'Invalid key' });
    }
    try {
      const raw = dbService.getConfig(key);
      if (raw == null) return res.json({ ok: true, value: null });
      try { return res.json({ ok: true, value: JSON.parse(raw) }); }
      catch { return res.json({ ok: true, value: raw }); }
    } catch (e) {
      return res.status(500).json({ ok: false, error: (e as Error).message });
    }
  });

  app.put('/api/ui-prefs/:key', (req, res) => {
    const { key } = req.params;
    if (!/^[a-zA-Z0-9_-]{1,64}$/.test(key)) {
      return res.status(400).json({ error: 'Invalid key' });
    }
...
function registerLeverageSubRoutes(app: Application): void {
  const setLeverageForInstId = async (
    apiKey: string, secretKey: string, passphrase: string,
    instId: string, lever: string, mgnMode: string, posSide?: string,
    acquireToken?: () => Promise<void>
  ): Promise<{ ok: boolean; error?: string }> => {
    const baseBody: Record<string, string> = { lever, mgnMode, instId };

    const sides: Array<string | undefined> = mgnMode === 'isolated'
      ? (posSide ? [posSide] : ['long', 'short'])
      : [undefined];

    let lastError: string | undefined;
    for (const side of sides) {
      const body: Record<string, string> = { ...baseBody };
      if (side) body.posSide = side;
      try {
        if (acquireToken) await acquireToken();
        const okxResp = await setLeverageStatic(apiKey, secretKey, passphrase, body);
        if (okxResp.code !== '0') {
          lastError = okxResp.msg || `OKX 错误码 ${okxResp.code}`;
        }
      } catch (e: unknown) {
  ...
  app.post("/api/system/set-leverage", asyncHandler(async (req: Request, res: Response) => {
    const { accountId, instId, lever, mgnMode, posSide } = req.body || {};
    if (!accountId) return res.status(400).json({ ok: false, error: '缺少 accountId' });
    if (!lever) return res.status(400).json({ ok: false, error: '缺少 lever' });
    if (!mgnMode) return res.status(400).json({ ok: false, error: '缺少 mgnMode' });

    const leverStr = String(lever);
    if (!/^\d+$/.test(leverStr) || parseInt(leverStr, 10) <= 0) {
      return res.status(400).json({ ok: false, error: '杠杆值必须是正整数' });
    }

    const lookup = findAccountById(String(accountId));
    if (!lookup) return res.status(404).json({ ok: false, error: '账户不存在' });
    ...
    if (!apiKey || !secretKey) {
      return res.status(400).json({ ok: false, error: '账户 API 密钥缺失' });
    }

    const mgnModeStr = String(mgnMode).toLowerCase();
    if (mgnModeStr !== 'cross' && mgnModeStr !== 'isolated') {
      return res.status(400).json({ ok: false, error: 'mgnMode 必须是 cross 或 isolated' });
    }

    const instIdStr = instId ? String(instId).trim().toUpperCase() : '';

    if (instIdStr) {
      if (!instIdStr.endsWith('-SWAP')) {
        return res.status(400).json({ ok: false, error: '杠杆设置仅支持 SWAP 合约交易对（instId 以 -SWAP 结尾）' });
      }
      const result = await setLeverageForInstId(apiKey, secretKey, passphrase, instIdStr, leverStr, mgnModeStr, posSide);
      if (result.ok) return res.json({ ok: true });
      return res.json({ ok: false, error: result.error });
    }

    const allSwapInstIds = InstrumentService.getAllSwapInstIds();
    if (allSwapInstIds.length === 0) {
      return res.status(400).json({ ok: false, error: '未找到 SWAP 合约交易对，请先初始化 InstrumentService' });
    }
    ...
    const fails: Array<{ instId: string; error: string }> = [];
    const total = allSwapInstIds.length;

    const writeProgress = (processed: number) => {
    ...
    try {
      const BATCH_INTERVAL_MS = 110;
      const BATCH_CONCURRENCY = 5;
      let nextTokenAt = Date.now();

      let tokenQueue = Promise.resolve();
      const acquireToken = (): Promise<void> => {
        const prev = tokenQueue;
        tokenQueue = prev.then(async () => {
          const waitMs = nextTokenAt - Date.now();
          if (waitMs > 0) await new Promise(r => setTimeout(r, waitMs));
          nextTokenAt = Math.max(nextTokenAt, Date.now()) + BATCH_INTERVAL_MS;
        });
        return prev;
      };
        ...
        const workers = Array.from({ length: Math.min(concurrency, tasks.length) }, async () => {
          while (cursor < tasks.length) {
            ...
            try {
              ...
              onDone({ ok: false, error: (e instanceof Error ? e : new Error(String(e))).message } as T, idx);
            }
          }
        });
      ...
      const tasks = allSwapInstIds.map(instId => () =>
        setLeverageForInstId(apiKey, secretKey, passphrase, instId, leverStr, mgnModeStr, posSide, acquireToken)
      );

      await runWithConcurrency(tasks, BATCH_CONCURRENCY, (result, idx) => {
        if (result.ok) {
          successCount++;
        } else {
          failCount++;
          fails.push({ instId: allSwapInstIds[idx], error: result.error || '未知错误' });
        }
        processedCount++;
        writeProgress(processedCount);
      });

      LogService.info('system', `批量设置杠杆完成: 总数=${total}, 成功=${successCount}, 失败=${failCount}`);
      ...
      res.end(JSON.stringify({
        ...
        failCount,
        fails: fails.length > 20 ? fails.slice(0, 20).concat({ instId: '...(更多失败省略)', error: `共 ${failCount} 个失败` }) : fails,
      }) + '\n');
    } catch (e: unknown) {
      const err = e instanceof Error ? e : new Error(String(e));
      LogService.error('system', `批量设置杠杆异常: ${err.message}`);
      res.end(JSON.stringify({
        type: 'done',
        ok: false,
        batch: true,
        total,
        okCount: successCount,
        failCount: failCount + (total - successCount - failCount),
        error: err.message,
        fails,
      }) + '\n');
    }
...
const IP_LOOKUP_TIMEOUT_MS = 5 * 1000;
...
function registerDiagnosticsSubRoutes(app: Application): void {
  ...
  app.get("/api/diagnostics", (_req, res) => {
    res.json({ ok: true, data: getDiagnosticsSnapshot() });
  });
}
...
function registerTunnelSubRoutes(app: Application): void {
  ...
  app.post("/api/tunnel/start", asyncHandler(async (req, res) => {
    const { port } = req.body;
    const localPort = typeof port === 'number' ? port : (getResolvedPort() ?? 3000);
...
function registerFormulaSubRoutes(app: Application): void {
  app.post("/api/evaluate-formula", asyncHandler(async (req, res) => {
    ...
    const needsS8 = /\b(s8|S8)\b/i.test(fml);
    if (needsS8) {
      if (!instId || typeof instId !== 'string') {
        return res.status(400).json({ ok: false, errorCode: 'MISSING_INST_ID' });
      }
      const sod = await fetchSodUtc8(instId);
      if (sod === null) {
        return res.json({ ok: false, errorCode: 'S8_UNAVAILABLE' });
      }
      variables.s8 = sod;
      variables.S8 = sod;
    }
    ...
    const needsS = formulaNeedsVar(fml, 's');
    if (needsS) {
    ...
    const needsCv = /\b(cv|CV)\b/i.test(fml);
    if (needsCv && instId) {
      const cv = InstrumentService.getCtVal(instId);
      if (cv !== null) {
        variables.cv = cv;
        variables.CV = cv;
      } else {
        return res.json({ ok: false, errorCode: 'MISSING_CTVAL' });
      }
    }

    const needsLs = /\b(ls|LS)\b/i.test(fml);
    if (needsLs && instId) {
      const ls = InstrumentService.getLotSz(instId);
      if (ls !== null) {
        variables.ls = ls;
        variables.LS = ls;
      } else {
        return res.json({ ok: false, errorCode: 'MISSING_LOTSZ' });
      }
    }
...
function registerPreviewSubRoutes(app: Application): void {
  app.post("/api/preview-orders", asyncHandler(async (req, res) => {
    const {
    ...
    const anyNeedsS8 = allFormulas.some(v => typeof v === 'string' && isFormula(v) && /\bs8\b/i.test(v.slice(1)));
    let sodUtc8: number | null = null;
    if (anyNeedsS8 && instId) {
      sodUtc8 = await fetchSodUtc8(instId);
    }
    ...
    const resolveVal = async (raw: string, extraVars?: Record<string, number>): Promise<number | null> => {
      ...
      const vars: Record<string, number> = { m: marketPrice ?? 0, M: marketPrice ?? 0, s: positionSize ?? 0, S: positionSize ?? 0, ...(sodUtc8 !== null ? { s8: sodUtc8, S8: sodUtc8 } : {}), ...extraVars };
    ...
    const tpMPair = parseMarginPair(takeProfitMargin);
    const slMPair = parseMarginPair(stopLossMargin);
    const tpM = tpMPair.trigger;
    const slM = slMPair.trigger;
    const tpMOrd = tpMPair.ord;
    const slMOrd = slMPair.ord;
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (isIntervalFormula) {
        if (i > 0) {
          const intervalN = i - 1;
          const vars: Record<string, number> = { m: marketPrice ?? 0, M: marketPrice ?? 0, s: positionSize ?? 0, S: positionSize ?? 0, ...(sodUtc8 !== null ? { s8: sodUtc8, S8: sodUtc8 } : {}), n: intervalN, N: intervalN };
      ...
      if (isFormula(contractSize)) {
        ...
        const vars: Record<string, number> = { m: marketPrice ?? 0, M: marketPrice ?? 0, s: positionSize ?? 0, S: positionSize ?? 0, ...(sodUtc8 !== null ? { s8: sodUtc8, S8: sodUtc8 } : {}), n, N: n, o: triggerPx, O: triggerPx };
        if (ctVal !== null) { vars.cv = ctVal; vars.CV = ctVal; }
        if (lotSz !== null) { vars.ls = lotSz; vars.LS = lotSz; }
      ...
      if (isRegular || isTrig) {
        ...
        if (!isMoveStop && !isMos) {
          if (tpM.value !== -1 && px > 0) {
            ...
            if (tpMOrd && tpMOrd.value !== -1) {
              order.tpOrdPx = parseFloat(calcTpSl(px, tpMOrd.value, tpMOrd.isPercent ?? tpM.isPercent, true).toFixed(8));
            }
          }
          if (slM.value !== -1 && px > 0) {
            order.slPrice = parseFloat(calcTpSl(px, slM.value, slM.isPercent, false).toFixed(8));
            if (slMOrd && slMOrd.value !== -1) {
              order.slOrdPx = parseFloat(calcTpSl(px, slMOrd.value, slMOrd.isPercent ?? slM.isPercent, false).toFixed(8));
            }
          }
  ...
  app.post("/api/preview-amend", asyncHandler(async (req, res) => {
    const {
    ...
    for (let idx = 0; idx < matchedOrders.length; idx++) {
      ...
      const isSpot = SymbolUtils.isSpotInstId(instIdStr);
  ...
  app.post("/api/preview-cancel", (req, res) => {
  ...
  app.post("/api/preview-close", (req, res) => {
    const {
...
function registerCoinIconSubRoutes(app: Application): void {
  app.get("/api/coin-icon/:symbol", asyncHandler(async (req: Request, res: Response) => {
    const symbol = req.params.symbol?.trim().toUpperCase();
    if (!symbol) {
      return res.status(400).json({ ok: false, error: 'MISSING_SYMBOL' });
    }
    const { url, notFound } = await CoinIconService.getImageUrlAsync(symbol);
    if (url) {
    ...
    const remainingCooldownMs = CoinIconService.getRateLimitRemainingMs();
    const retryAfterSec = Math.max(1, Math.ceil(remainingCooldownMs / 1000));
    res.setHeader('Retry-After', String(retryAfterSec));
    return res.status(429).json({ ok: false, error: 'RATE_LIMITED', retryAfter: retryAfterSec });
  }));

  app.get("/api/instruments/tick-sizes", (_req: Request, res: Response) => {
    const all = InstrumentService.getAll();
    const map: Record<string, number> = {};
    for (const inst of all) {
      const tick = parseFloat(inst.tickSz || '');
      if (Number.isFinite(tick) && tick > 0) {
        map[inst.instId] = tick;
      }
    }
