...
import { validateModuleConfigMiddleware } from '../../middleware/inputValidation.ts';
...
export function registerCrudRoutes<T extends CrudConfigRecord>(
  ...
  app.post(cfgPath, (req, res) => {
    try {
      ...
      const newConfig = createConfig(req.body, id);
  ...
  app.post(`/api/${basePath}/start/:configId`, validateModuleConfigMiddleware, (req, res) => {
