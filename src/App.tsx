...
export default function App() {
  ...
  const loadHistoryData = useCallback(async () => {
    ...
    try {
      const accounts = historyAccListRef.current;
      if (accounts.length === 0) return;

    const fetchJson = async (url: string, body: Record<string, unknown>): Promise<{ ok: boolean; data?: unknown[] }> => {
    ...
    try {
      for (const acc of accounts) {
        ...
        const [normalData, algoData] = await Promise.all([
          fetchJson('/api/orders/history', { accountId, instType: 'SWAP', limit: 100 }),
          fetchJson('/api/orders/algo-history', { accountId, instType: 'SWAP', limit: 100 }),
        ]);
        if (normalData.ok && Array.isArray(normalData.data)) allOrders.push(...(normalData.data as Order[]));
        if (algoData.ok && Array.isArray(algoData.data)) allOrders.push(...(algoData.data as Order[]));

        const exchange = String(historyAccountIdExchangesRef.current[accountId] || '').toUpperCase();
        if (exchange === 'OKX') {
          const posData = await fetchJson('/api/positions/history', { accountId, instType: 'SWAP', limit: 100 });
          if (posData.ok && Array.isArray(posData.data)) allPositions.push(...(posData.data as HistoryPosition[]));
        }
      }
    } catch (e) {
