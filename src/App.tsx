...
import { Wallet, FilePlus2, Edit3, Trash2, XCircle, ArrowLeftRight, Bot, Key, Activity, CircleDollarSign, Settings, MessageCircle, ArrowLeft, ArrowRight, RefreshCcw, TrendingUp } from 'lucide-react';
...
import type { AppConfig, LogEntry, Order } from './types/index.ts';
...
const NavItem = React.memo(({ id, isActive, icon: Icon, label, onClick, isCollapsed }: { id: string, isActive: boolean, icon: React.ComponentType<{ className?: string, strokeWidth?: number }>, label: string, onClick: (id: string) => void, isCollapsed: boolean }) => {
  const content = (
    <button
      onClick={() => onClick(id)}
      className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-medium mb-1 ${
        isActive
...
function historyPosDedupeKey(p: HistoryPosition): string {
  const idPart = p._accountId ?? p._account;
  const posPart = p.posId ?? `np:${p.instId}:${p.type ?? ''}:${p.cTime ?? ''}:${p.closeAvgPx ?? ''}`;
  return `${idPart}|${posPart}|${p.uTime ?? ''}`;
}

function mergeHistoryPositions(prev: HistoryPosition[], incoming: HistoryPosition[]): HistoryPosition[] {
  const seen = new Set<string>();
  const merged: HistoryPosition[] = [];
  for (const p of [...prev, ...incoming]) {
    if (!p) continue;
    const key = historyPosDedupeKey(p);
    if (seen.has(key)) continue;
    seen.add(key);
    merged.push(p);
  }
  return merged.sort((a, b) => {
    const ta = parseInt(String(a.uTime || a.cTime || '0')) || 0;
    const tb = parseInt(String(b.uTime || b.cTime || '0')) || 0;
    return tb - ta || String(a.posId ?? '').localeCompare(String(b.posId ?? ''));
  });
}
...
export default function App() {
  ...
  useEffect(() => {
    const handleStrategyTriggered = (e: Event) => {
      const detail = (e as CustomEvent<{ strategyName?: string }>).detail;
      const name = detail?.strategyName;
      if (!name) return;
      showToast(t('diy.ui.strategyTriggered', { name }), 'success');
    };
    window.addEventListener('strategy-triggered', handleStrategyTriggered);
    return () => window.removeEventListener('strategy-triggered', handleStrategyTriggered);
  }, [showToast, t]);
  ...
  const historyAccListRef = useRef(accList);
  const historyAccountIdExchangesRef = useRef(accountIdExchanges);
  const historyLoadingRef = useRef(false);
  const historyReloadPendingRef = useRef(false);
  const HISTORY_RECHECK_DELAYS = [6000, 10000, 14000, 20000];
  const MAX_HISTORY_RECHECK = HISTORY_RECHECK_DELAYS.length;
  const historyRecheckCountRef = useRef(0);
  const historyRecheckTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  useEffect(() => {
    historyAccListRef.current = accList;
    historyAccountIdExchangesRef.current = accountIdExchanges;
  }, [accList, accountIdExchanges]);

  const histPosAfterRef = useRef<Record<string, { after: string; hasMore: boolean }>>({});
  const [histPosLoadingMore, setHistPosLoadingMore] = useState(false);
  const histPosLoadingMoreRef = useRef(false);

  const loadMoreHistoryPositions = useCallback(async () => {
    if (histPosLoadingMoreRef.current) return;
    if (historyLoadingRef.current) return;
    histPosLoadingMoreRef.current = true;
    setHistPosLoadingMore(true);
    const cursorsAtStart = { ...histPosAfterRef.current };
    try {
      const cursors = cursorsAtStart;
      const accountIds = Object.keys(cursors).filter(id => cursors[id].hasMore);
      if (accountIds.length === 0) return;
      const allPositions: HistoryPosition[] = [];
      const nextCursors: Record<string, { after: string; hasMore: boolean }> = {};
      for (const accountId of accountIds) {
        const exchange = String(historyAccountIdExchangesRef.current[accountId] || '').toUpperCase();
        if (exchange !== 'OKX') continue;
        const c = cursors[accountId];
        ...
        try {
          const res = await fetch('/api/positions/history', {
            ...
            body: JSON.stringify({ accountId, instType: 'SWAP', limit: 100, after: c.after }),
            signal: ctrl.signal,
          });
          const json = await res.json().catch(() => ({ ok: false }));
          if (!json.ok || !Array.isArray(json.data)) continue;
          const page = json.data as HistoryPosition[];
          if (page.length > 0) allPositions.push(...page);
          const pageSize = 100;
          const last = page[page.length - 1];
          const newAfter = last ? String(last.uTime || last.cTime || '') : '';
          nextCursors[accountId] = { after: newAfter, hasMore: page.length >= pageSize };
        } finally {
          clearTimeout(timer);
        }
      }
      if (allPositions.length === 0) return;
      setHistoryPositions(prev => mergeHistoryPositions(prev, allPositions));
      const cursorsNow = histPosAfterRef.current;
      const cursorsUntouched = accountIds.every(id => cursorsNow[id] === cursorsAtStart[id]);
      if (cursorsUntouched) {
        histPosAfterRef.current = { ...cursorsNow, ...nextCursors };
      }
    } finally {
      histPosLoadingMoreRef.current = false;
      setHistPosLoadingMore(false);
    }
  }, []);

  const histPosHasMore = Object.values(histPosAfterRef.current).some(c => c.hasMore);

  const loadHistoryData = useCallback(async (targetIdx?: number | null) => {
    if (historyLoadingRef.current) {
      historyReloadPendingRef.current = true;
      return;
    }
    const onlyIdx = targetIdx !== undefined ? targetIdx : (bumpHistoryTargetRef.current ?? null);
    if (targetIdx === undefined) bumpHistoryTargetRef.current = null;
    historyLoadingRef.current = true;
    try {
      let accounts = historyAccListRef.current;
      if (onlyIdx != null) accounts = accounts.filter(a => a._account === onlyIdx);
      if (accounts.length === 0) return;

    const fetchJson = async (url: string, body: Record<string, unknown>): Promise<{ ok: boolean; data?: unknown[]; stale?: boolean }> => {
      const ctrl = new AbortController();
      const timer = setTimeout(() => ctrl.abort(), 15000);
      try {
    ...
    try {
      for (const acc of accounts) {
        const accountId = acc._accountId || acc.id;
        if (!accountId) continue;

        const [normalData, algoData] = await Promise.all([
          fetchJson('/api/orders/history', { accountId, instType: 'SWAP', limit: 100, refresh: onlyIdx != null }),
          fetchJson('/api/orders/algo-history', { accountId, instType: 'SWAP', limit: 100, refresh: onlyIdx != null }),
        ]);
        if (normalData.ok) {
          if (normalData.stale) anyStale = true;
          if (Array.isArray(normalData.data)) allOrders.push(...(normalData.data as Order[]));
        } else {
          anyStale = true;
        }
        if (algoData.ok) {
          if (algoData.stale) anyStale = true;
          if (Array.isArray(algoData.data)) allOrders.push(...(algoData.data as Order[]));
        } else {
          anyStale = true;
        }
        ...
        const exchange = String(historyAccountIdExchangesRef.current[accountId] || '').toUpperCase();
        if (exchange === 'OKX') {
          const posData = await fetchJson('/api/positions/history', { accountId, instType: 'SWAP', limit: 100, refresh: onlyIdx != null });
          if (posData.ok) {
            if (posData.stale) anyStale = true;
            const posList = Array.isArray(posData.data) ? (posData.data as HistoryPosition[]) : [];
            allPositions.push(...posList);
            if (posList.length > 0) {
              const PAGE_SIZE = 100;
              const last = posList[posList.length - 1];
              histPosAfterRef.current[accountId] = {
                after: String(last.uTime || last.cTime || ''),
                hasMore: posList.length >= PAGE_SIZE,
              };
            } else {
              histPosAfterRef.current[accountId] = { after: '', hasMore: false };
            }
          } else {
            anyStale = true;
          }
    ...
    allOrders.sort((a, b) => {
      const timeA = parseInt(a.uTime || a.orderTime || '0');
      const timeB = parseInt(b.uTime || b.orderTime || '0');
      return timeB - timeA;
    });
    if (onlyIdx != null) {
      setHistoryOrders(prev => [...prev.filter(o => (o as Order)?._account !== onlyIdx), ...allOrders].sort((a, b) => {
        const ta = parseInt(a.uTime || a.orderTime || '0');
        const tb = parseInt(b.uTime || b.orderTime || '0');
        return tb - ta;
      }));
      setHistoryPositions(prev => {
        const remaining = prev.filter((p) => (p as HistoryPosition)?._account !== onlyIdx);
        return mergeHistoryPositions(remaining, allPositions);
      });

      if (anyStale && historyRecheckCountRef.current < MAX_HISTORY_RECHECK) {
        const nextDelay = HISTORY_RECHECK_DELAYS[historyRecheckCountRef.current];
        historyRecheckCountRef.current += 1;
        historyRecheckTimerRef.current = setTimeout(() => {
          historyRecheckTimerRef.current = null;
          void loadHistoryData(onlyIdx);
        }, nextDelay);
      }
    } else {
  ...
  useEffect(() => {
    ...
    const delay = historyVersion === 0 ? 0 : 500;
    historyRecheckCountRef.current = 0;
    if (historyRecheckTimerRef.current) {
      clearTimeout(historyRecheckTimerRef.current);
      historyRecheckTimerRef.current = null;
    }
  ...
  }, [snapshotLoaded, historyVersion, loadHistoryData]);

  useEffect(() => {
    if (!snapshotLoaded) return;
    const timer = setInterval(() => {
      void loadHistoryData(null);
    }, 60000);
    return () => clearInterval(timer);
  }, [snapshotLoaded, loadHistoryData]);
    ...
    <ErrorBoundary onError={handleError}>
      ...
      <aside className={`border-r border-border-default px-2 pt-2 pb-4 flex flex-col shrink-0 h-full transition-all duration-300 ${isSidebarCollapsed ? 'w-[65px]' : 'w-[180px]'} group`}>
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto hidden-scrollbar">
          <NavItem id="market" icon={CircleDollarSign} label={t('nav.market')} isActive={activeModule === 'market'} onClick={handleSetActiveModule} isCollapsed={isSidebarCollapsed} />
          <NavItem id="monitor" icon={TrendingUp} label={t('nav.monitor')} isActive={activeModule === 'monitor'} onClick={handleSetActiveModule} isCollapsed={isSidebarCollapsed} />
          <NavItem id="positions" icon={Wallet} label={t('nav.positions')} isActive={activeModule === 'positions'} onClick={handleSetActiveModule} isCollapsed={isSidebarCollapsed} />
          <NavItem id="place" icon={FilePlus2} label={t('nav.place')} isActive={activeModule === 'place'} onClick={handleSetActiveModule} isCollapsed={isSidebarCollapsed} />
          <NavItem id="amend" icon={Edit3} label={t('nav.amend')} isActive={activeModule === 'amend'} onClick={handleSetActiveModule} isCollapsed={isSidebarCollapsed} />
          <NavItem id="cancel" icon={Trash2} label={t('nav.cancel')} isActive={activeModule === 'cancel'} onClick={handleSetActiveModule} isCollapsed={isSidebarCollapsed} />
          <NavItem id="close" icon={XCircle} label={t('nav.close')} isActive={activeModule === 'close'} onClick={handleSetActiveModule} isCollapsed={isSidebarCollapsed} />
          <NavItem id="margin" icon={ArrowLeftRight} label={t('nav.margin')} isActive={activeModule === 'margin'} onClick={handleSetActiveModule} isCollapsed={isSidebarCollapsed} />
          <NavItem id="diy" icon={Bot} label={t('nav.diy')} isActive={activeModule === 'diy'} onClick={handleSetActiveModule} isCollapsed={isSidebarCollapsed} />
          <NavItem id="apikeys" icon={Key} label={t('nav.apikeys')} isActive={activeModule === 'apikeys'} onClick={handleSetActiveModule} isCollapsed={isSidebarCollapsed} />
          <NavItem id="settings" icon={Settings} label={t('nav.settings')} isActive={activeModule === 'settings'} onClick={handleSetActiveModule} isCollapsed={isSidebarCollapsed} />
          <NavItem id="contact" icon={MessageCircle} label={t('nav.contact')} isActive={activeModule === 'contact'} onClick={handleSetActiveModule} isCollapsed={isSidebarCollapsed} />
        </nav>
        ...
        <div className="flex-1 flex flex-col overflow-y-auto custom-scrollbar">
          ...
          <div className={`flex flex-col flex-1 p-4 ${activeModule === 'market' ? '' : 'hidden'}`}>
            <MarketAnalysisView
              ...
              historyPosHasMore={histPosHasMore}
              historyPosLoadingMore={histPosLoadingMore}
              onLoadMoreHistoryPositions={loadMoreHistoryPositions}
          ...
          <div className={`flex flex-col flex-1 min-h-0 p-6 ${activeModule === 'monitor' ? '' : 'hidden'}`}>
            <MarketMonitorModule isActive={activeModule === 'monitor'} />
          </div>
      ...
      <LogModal
        ...
        accountNames={config?.accountNames as Record<string, string> | undefined}
        accountColors={config?.accountColors as Record<string, string> | undefined}
      />
