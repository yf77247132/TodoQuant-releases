declare global {
  ...
  var ORDERS_CACHE:
    | {
        ...
        getRestMissingAt: (accountIdx: number, orderId: string) => number;

        markRestMissing: (accountIdx: number, orderId: string, at?: number) => void;

        clearRestMissing: (accountIdx: number, orderId: string) => void;
      }
    | undefined;
}
