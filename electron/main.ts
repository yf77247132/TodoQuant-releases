import { app, BrowserWindow, powerMonitor, ipcMain, Menu, shell, screen, Notification, dialog } from 'electron';
...
function startBackend(port: number): ChildProcess {
  ...
  const integrityFiles = [
    { path: path.join(distServerDir, 'server.cjs'), hash: path.join(distServerDir, 'server.cjs.hash') },
  ];
  ...
  for (const { path: filePath, hash: hashFilePath } of integrityFiles) {
    if (!require('fs').existsSync(hashFilePath)) {
      if (useCompiledServer) {
        console.error(`[Security] INTEGRITY HASH MISSING (production): ${hashFilePath}`);
        throw new Error(`Integrity hash file missing: ${hashFilePath}`);
      }
      continue;
    }
...
function setupIpcHandlers(): void {
  ...
  ipcMain.handle('open-external', (_event, url: string) => {
    if (typeof url !== 'string' || !/^https?:\/\//i.test(url)) {
      console.warn(`[Security] open-external rejected non-http(s) url: ${String(url).slice(0, 100)}`);
      return;
    }
...
app.whenReady().then(main).catch((err) => {
  ...
  try {
    dialog.showErrorBox('TodoQuant 启动失败', `${message}\n\n请将以上信息反馈给开发者。`);
  } catch {  }
  app.exit(1);
});
